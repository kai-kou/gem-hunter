...
- **スプリント開発とリファインメントを単独実行しない。例外なし**。メインセッションは **オーケストレーターに徹し**（委譲・統合・最終判断・ユーザー報告のみ）、実装コードを自分で書かない。sp 別の編成（`sp:1`〜`2` は最小 2 役割 / `sp:3`〜`5` は 3 役割以上 / `sp:8` は着手前に議論型 1 ラウンド）の **正本は `docs/rules/session-sprint-rules.md` §2**
