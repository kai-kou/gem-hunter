...
**`Stop hook feedback:` で始まる続行ターンでは完了報告を再掲しない**（確認結果を 1〜3 行。`[report-format]` があるときだけ簡潔に書き直す）。切り分けの正本は同ファイル §1.2（base#543）。
...
`http-client-consolidation-rules.md`（**`tools/github_rest.py` / `tools/github_api.py` 等の共通 HTTP クライアントモジュールを新設・改修するとき、または新しいツールで GitHub REST / gh CLI 呼び出しを書く前に必読**・#825）/
...
> `--fix` を実行）。本リポジトリは R-1 ルーティン稼働のため Hot 化済み。
...
- ローカル実行では gh がフル機能で動く（シムは即 exec でパススルー・repo 指定は `-R kai-kou/gem-hunter`）
...
### 🔴 チーム編成とスプリントの締め方（飼い主の恒久指示・2026-08-19）

- **スプリント開発とリファインメントを単独実行しない。例外なし**。メインセッションは **オーケストレーターに徹し**（委譲・統合・最終判断・ユーザー報告のみ）、実装コードを自分で書かない。**既定の協調モードと sp 別の役割数の正本は `docs/rules/session-sprint-rules.md` §2**（しきい値をここに複製しない）
- **マージしたら必ずスプリントレビューとレトロスペクティブをチームで実施し記録を残す**（対象 Issue コメント + `retrospective` スキル）。**手順の正本は `.claude/skills/pr-review-watcher/SKILL.md` Step 7**。実施前に完了報告しない
...
配線は `.claude/settings.json`、実体は `.claude/hooks/*.sh`（全 26 スクリプト）。
...
| 実行ガード | `pre-tool-use-router.sh` → `pre-git-push-check.sh` / `pre-pr-create-check.sh` / `pre-cloudflare-destructive-check.sh` / `pre-cloudflare-mcp-allowlist-check.sh`、`pre-file-tool-env-guard.sh` / `pre-merge-layer1-check.sh` / `permission-request-auto-allow.sh` | main 直 push ブロック・PR 作成前チェック・Cloudflare Workers スクリプトへの破壊的操作ブロック（#613/#615）・Cloudflare MCP アローリスト（#56）・.env アクセスブロック（Bash / ファイルツール双方）・作業ツリー外への Bash 書き込みブロック（`lib/workspace_write_guard.py`・base#578）・マージ前 Layer 1 セルフレビュー実施の非ブロッキング確認（base#512）・`.claude/` 配下の編集自動許可         |
| 事後検証   | `post-tool-use-validate.sh` / `post-merge-publish-check.sh` / `post-review-write-mark.sh` / `post-pr-confirm-mark.sh` / `post-sprint-review-deploy-check.sh` / `post-tool-use-failure.sh` / `subagent-stop.sh`                                                   | 成果物バリデーション拡張点（既定 no-op）・main マージ直後の公開反映起動・Layer 1 セルフレビュー提出のマーカー記録・PR 存在確認済みのマーカー記録（`stop-pr-check.sh` の毎ターン差し戻しを抑止・base#543）・スプリントレビュー後のデプロイ判定・gh プロキシ起因エラーの案内・サブエージェント異常終了の自己修正 |
...
表を書いたら `python3 tools/check_markdown_table_columns.py --changed`（`--under` の注意は上と同じ）も実行する。GFM の列数不整合と、**行末パイプの後ろに書いて GitHub が黙って落とす内容** を検出する。
