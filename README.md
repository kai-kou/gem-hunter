...
- **汚染を落としてから数える**: チュートリアル・テンプレート・ミラーの類を除外し、同一リポジトリを指す複数パッケージはリポジトリ単位で重複排除する。この 2 段を通した結果が配信プールの 62,483 リポジトリ
- **配信はレジストリ別シャードの静的アセット**: DB を持たない原則を崩さずに 6 万件を引くため、レジストリごとに分割した静的 JSON を isolate の cold start で並列取得して単一の `Map` にまとめ、以降はメモリ上で join する。初期化は singleton promise にして、cold start が重なっても取得が多重化しないようにした
