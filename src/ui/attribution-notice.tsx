...
type AttributionNoticeLabels = {
   ...
   *
   * 🔵 **Issue #310 で「今日の Gem」（`home.digest.attribution`）の文言を再刷新**: star 数は
   * バッチ生成時点で GitHub API から取り直すため、もはや Ecosyste.ms 由来の古い参考値ではない
   * （詳細画面とはユーザーが開いた時刻の違いでのみ差が生じうる）。「参考値」の位置づけが残るのは
   * 利用パッケージ数（`dependentCount`・引き続き Ecosyste.ms 由来）だけである。Gem 一覧
   * （`gems.attribution`）は Gem Index の鮮度が対象外（Issue #310 スコープ注記）のため、
   * 両方が参考値である従来の文言のままにしている。
   *
