 ...
 * ⚠️ **この呼び出しは第 2 段の検索インデックス構築（実測 cold 2.78s）自体は避けられない**
 * （実測で確認済み）。避けられているのは全件走査と緩和カウント（warm 約 10ms）だけで、
 * 構築まで避けるにはポート契約の変更（`meta()` の追加・`tokens` の判別ユニオン化）が要る。
 * **本 PR では扱わず別 Issue**（ポート契約を同じ PR で二度動かさない・親裁定 2026-08-22）。
 *
 ...
 * 実測 +2〜5ms（warm・0 件のときだけ）で許容範囲のため、**上の構築回避と同じ別 Issue** にまとめる。
 *
