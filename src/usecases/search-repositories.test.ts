...
describe('searchRepositories', () => {
  ...
  it('search は常に1回しか呼ばない（gem-index 撤去後は sort 値に関わらず単発呼び出し）', async () => {
    const received: SearchQuery[] = []
    const searchRepositories = makeSearchRepositories({ repos: fakePort(received) })
  ...
  it('撤去済みの sort=gem-index は SortOrder の検証で拒否される（parse は tryParse と異なりフォールバックしない）', async () => {
    const received: SearchQuery[] = []
    const searchRepositories = makeSearchRepositories({ repos: fakePort(received) })

    await expect(searchRepositories({ keyword: 'react', sort: 'gem-index' })).rejects.toThrow(
