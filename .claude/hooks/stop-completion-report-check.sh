...
#     （ただし後述のマーカー記録はこのターンでも必ず行う・base#633）
#
...
#
# 【Issue base#633】上記マーカーが実運用でほぼ機能していなかった根本原因の修正。base#543 の実装は
...
#
# 【Issue #69・本リポジトリ固有】上記に加え、Sprint Goal: を含む PR マージ後に
# Sprint Review / Retrospective の実施証跡が無いまま終了しようとしているセッションを検知する
# （classify_sprint_evidence・session-sprint-rules.md「スプリント完了時の必須事項」の機械補助）。
#
...
# 【base#633】判定条件から MERGE_RE だけを外した（PR_REF_RE は必須のまま維持する）。§1 テンプレートの
...
[PR #11](https://example.com/pull/11) をマージしました" "yes"
  # base#633: PR リンクはあるがマージ語を含まない完了報告もマーカー対象（テンプレート準拠なら拾う）
  assert_proper "## ✅ 完了報告
**ご依頼**: ログ出力の改善。
...
[PR #12](https://example.com/pull/12) / ブランチ \`feat/log\`" "yes"
  # base#633: PR 参照が無い適正報告ではマーカーを立てない。立てると、同一セッションの後続タスクが
...
**できるようになったこと**: 失敗条件が特定できたにゃ。" "no"
  ...
  if [[ -n "$e2e_dir" ]]; then
    ...
    # ④【base#633 回帰テスト】続行ターン（stop_hook_active=true）で出した適正な完了報告も
    ...
    if [[ -f "$marker_path" ]]; then :; else echo "FAIL: e2e④: 続行ターンで出した適正報告がマーカー化されていない（base#633 の回帰）"; fail=1; fi
    ...
    # ⑦【base#633 回帰テスト】PR 参照の無い適正報告ではマーカーを立てない → 同一セッションの
...
**できるようになったこと**: 失敗条件が特定できたにゃ。'
    ...
    # --- Sprint Review / Retrospective 証跡ブロック（Issue #69・本リポジトリ固有）の e2e ---
...
# 【base#633】true でも即 exit しない。続行ターンこそ本ベースで完了報告が出る最頻経路であり、
# ここで抜けると「適正な完了報告済み」マーカーが永久に立たず、後続の通知 wake で二重報告を
# 引き起こす。マーカー記録・Sprint 証跡チェックは必ず通し、nudge（hook_block）だけを下でスキップする。
...
if [[ -n "$last_text" ]]; then
  ...
  # 基準ディレクトリの解決は他のマーカー系フック（post-pr-confirm-mark.sh 等）と同じ `--git-dir` に揃える。
  ...
  # ここは stop_hook_active の値によらず必ず通る（続行ターンの完了報告を取りこぼさない・base#633）。
  ...
  if [[ "$stop_hook_active" != "true" ]] && [[ "$(classify_text "$last_text")" == "nudge" ]]; then
...
ただし **このセッションで既に §1 準拠の完了報告を出している** 場合は本チェックの誤発火なので、完了報告を再掲も書き直しもしないこと（同じ報告が 2 回並ぶ・base#633）。その場合は確認した事実を 1〜3 行で返して終える（completion-report-rules.md §1.2）。"
    fi
  fi
fi
