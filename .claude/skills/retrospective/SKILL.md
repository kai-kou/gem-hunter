...
🔴 **実行結果の記録を必須とする（#715）**: 上記 `jq` コマンドは **必ず実行し**、その出力を Step 3 完了後の記録（下記表の「前回持ち越し分の合流」行）に残す。ヒットが 0 件だった場合も「0 件」と明記する（「結論だけ書いて実行していない」状態と区別できないと、`docs/rules/lessons-core.md` が CRITICAL 1 と呼ぶ自己ロックの再発を検知できない）。
