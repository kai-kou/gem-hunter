...
反例の作り方は `code-review` スキルの観点「判定ロジックの適用範囲」の 4 チェック（マッチのアンカー / 対象層の取り違え / 区別テストの有無 / 倒れる向き）を使う（#244・同じ「広すぎる一致」を狙う観点なので指示文で相互参照する）。

🔴 **事実確認（`fact_check` 相当）のレンズには、次の 2 観点を必ず指示文に含める**（#373・実測: LP の
「初回はすべてが新着として出ます」が実装と逆だったのに 5 レンズ × 2 ラウンドを完全通過した）。

1. **状態分岐ごとの挙動を検証する**（機能の有無で止めない）。「その機能が存在するか」ではなく
   「**どの状態でどう振る舞うか**」を実装で確かめる: 初回訪問 / 2 回目以降・データ 0 件 / 1 件 / 多数・
   正常 / エラー・未ログイン / ログイン済み・env 未供給 / 供給済み。**主張が成り立たない状態が 1 つでも
   あれば指摘する**（「env が供給された瞬間に虚偽化する」類の主張を含む）。
2. **成果物が掲載している画像を実際に Read し、本文の主張と照合する**（`.webp` / `.png` / スクリーンショット等）。
   画像は本文の反証になりうる（バッジなしの画面を載せたまま「バッジが付きます」と書く等）。
   コードだけを読んで済ませない。視覚レンズ任せにもしない（見え方の評価と本文との突き合わせは別作業）。
...
- プロンプトに必ず含める: ① 自分の name とレンズ（lens） ② 対象の **絶対パス** ③ 投稿コマンド
  ...
  ⑦ 🔴 **並行安全プリアンブル（プロンプト先頭に実テキストで展開する）**: 起動前に
  `docs/rules/agent-team-summary.md` の「並行安全プリアンブル」節を Read し、**その節のコードブロックの
  中身を実テキストのまま** プロンプト先頭へ貼る（パス参照・節名の記載だけでは実行時に解決されず、
  サブエージェントには何も届かない）。参加者は **読み取り専用** なので、貼った直後に
  `ファイルの編集も禁止する。` を 1 行足し、続けて「指摘はホワイトボードへの投稿でのみ行う」と書く。
  禁止文言そのものを本ファイルへ複製しない（二重管理の再発防止・#816。議論フェーズの参加者が担当外で
  リポジトリを直接編集し、統合フェーズ（fan-out 実装）の別担当と作業が二重化した実例あり・Issue #200）

全参加者の完了通知を待つ。**待ち方は「そのターンを終えること」**（通知は後のターンで届く）。
`sleep` ループ・`ReadNotifications` 連打・`TaskGet` / `/tasks` の進捗確認など **手段を問わずポーリングしない**。
「完了待機中」の実況も出さない（`agent-team.md` §F-8「『待つ』の作法」）。
揃ったら `render` を実行し投稿を確認する（`list <id> --round 1 --json` で participant 全員分の post があること）。
...
🔴 再開指示にも **並行安全プリアンブルの実テキスト + `ファイルの編集も禁止する。`** を先頭へ展開する
（`docs/rules/agent-team-summary.md` の該当節を Read して貼る）。トランスクリプト再開でも参加者の
規律が Step 1 のまま維持されている保証はないため、ラウンドごとに貼り直す。

全員の完了通知を待ち（Step 1 と同じ作法。ポーリング・待機実況をしない）、`render` する。
...
1. **参加者無応答**: 他の参加者の通知が揃ったあとも 1 名分だけ届かない状態が続く場合（目安 10 分。
   **経過時間を見るためにポーリングしない** — 他の作業を進めて自然にターンが進んだ時点で判定する）、
   SendMessage で 1 回だけ状況確認を送る。それでも応答がなければその参加者を欠席扱いにして進行し、
   verdict にその旨を記録する。
