...
- 🔴 **SYNC_PATHS 配下（`docs/rules` / `.claude/*` / `tools` / `scripts` / `.mcp.json` / `.claude/settings.json` 等）も無条件上書きしない**。
  ...
  ワークツリーに書かれない**（`tools/merge_three_way.py` が JSON 構文・重複キー・`hooks.*` など
  順序依存配列の相対順序まで検証し、通らなければ採用しない）。ベースに存在しないファイル
  （下流独自のルール・スキル）は一切触られない。
...
`apply-to-repo.sh` は **実行に使ったコピー自身**（`$0` の実体）にドリフト検査（§2.7）等が
配線されているかどうかで挙動が変わる。🔴 **対象リポジトリに `scripts/apply-to-repo.sh` が
...
対象リポジトリ内の **自分自身のコピー** を直接実行する。このコピーは実行時に内部の
`fetch_base()` がベースを自前で clone するため、事前に手動で clone する必要はない。
このコピーにはドリフト検査（§2.7）が配線済みなので、検査も同じコマンドで走る。

```bash
bash scripts/apply-to-repo.sh            # オプションは末尾に付与（例: --prune --tz Asia/Tokyo）
```

**b. 初回適用（対象リポジトリに `scripts/apply-to-repo.sh` がまだ無い）**

自分自身のコピーが無いため、ベースを一時ディレクトリへ clone し、**そちらの**
`scripts/apply-to-repo.sh`（ベース側のコピー・ドリフト検査は未配線）を実行して初期導入する。
...
この初回適用でドリフト検査が走らないのは正常（§2.7）。この実行で `scripts/apply-to-repo.sh` が
対象リポジトリへ配置されるため、**次回以降は a. の経路に切り替わり検査も有効になる**。
...
- 主なオプション: `--prune`（`modules.yaml` で無効化したモジュール資産を除去）/ `--tz` / `--overwrite-project`（CLAUDE.md 等も上書き）/ `--check-updates`（アップデート内容の表示のみ・適用なし）/ `--dry-run`（適用対象と 4 分岐の判定結果だけ表示）/ `--no-merge`（3 方向マージを行わず旧来の無条件上書きに戻す退避経路）。`--base owner/repo` はベースの取得元スラッグを差し替える場合のみ使う（ローカルパス不可）。
...
- 表示された **手動手順（base-update-notes のエントリ）** は、§3.2 の `*.base` 取り込みと同様に
...
🔴 **ドリフト検査が走るかどうかは §2 のどちらの経路で実行したかで決まる**（Issue #889。
「適用のたびに自動実行され追加のコマンド実行は不要」という記述は §2-a の経路にのみ当てはまる。
§2-b の初回適用は **必ずスキップされる**。「手順どおり実行したのに検査が走らない」は §2-b を
踏んでいるときの正常動作であり、故障ではない）。

- **§2-a（2 回目以降・対象リポジトリ自身の `scripts/apply-to-repo.sh` を実行）**: 実行のたびに
  `tools/check_apply_base_drift.py` を自動実行し、「本リポジトリ固有の拡張行（ベース由来ファイルへの
  インライン追記）が今回の同期で消えていないか」を機械判定する。追加のコマンド実行は不要
  （§2-a のコマンド 1 発に含まれる）。python3 が無い環境・`--dry-run` 実行時・スナップショット
  取得失敗時は、適用ログの `── 本リポジトリ固有拡張のドリフト検査 ── SKIP: <理由>` 行と、
  末尾の完了サマリーの「ドリフト検査: SKIP（<理由>）」行に理由が出る（`scripts/apply-to-repo.sh`
  §3.4 実装）。
- **§2-b（初回適用・ベース側の `scripts/apply-to-repo.sh` を実行）**: 🔴 **実測（Issue #889・
  ベースリポジトリ `main` を `git clone` して `grep -n drift scripts/apply-to-repo.sh` で確認・
  ヒット 0 件）: ベース側のコピーにはドリフト検査そのものが一切配線されていない。** §2-a の
...
§2-a 実行時の判定は 3 通り: **drift なし**（`RESULT: clean`）/
...
- 🔴 **`.base-latest` を「ベース版で全面採用」する前に、下流側ファイルの Issue 参照（`#N`）・実測表・
  「例外」節をスキャンする**（`docs/rules/lessons/docs-sync.md` L-168）。ベース側に対応物が無ければ、
  それは「独自記述なし」ではなく **下流固有の決定** である。2026-09-10 の適用では見送り Try ログの記録手順・
  単一固定ブランチ制約の例外・散らばり閾値の実測根拠が、この判断ミスで消えた（`run_checks.sh` は緑のまま通過した）。
- 🔴 **配布された検査ツール・ガードの付属 self-test が緑であることを、動作の根拠にしない**（同 L-168）。
  無人実行中に静かに効かなくなる種類のもの（フック・ガード）は、配線する前に敵対的入力を自分で投げて実測する。
...
- §2-a を実行し、ドリフト検査結果が `drift なし` 以外（`drift 検出` / `判定不能`）だった場合、
  対応した内容（復元した / false positive と判断した理由）を報告に 1 行含める。
- §2-a を実行し、python3 不在・`--dry-run`・スナップショット取得失敗のいずれかで検査が
  SKIP された場合は、その理由を報告に 1 行含める。
- §2-b（初回適用）を実行した場合は、ログにドリフト検査への言及が一切無いのは正常（§2.7 参照）
  と 1 行示し、次回以降は §2-a に切り替わり検査が有効になることも添える。
- **セッション途中の適用ではフック（`.claude/hooks/`）・`settings.json` の変更は再読込されない**
  （起動時スナップショット・公式仕様）。完了報告に「フック・settings の変更は新セッションから
  有効になる」旨を 1 行添える（`scripts/apply-to-repo.sh` の完了サマリーにも同じ注意が出力される）。
