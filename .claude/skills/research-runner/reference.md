...
```bash
SKIP=$(gh issue view {ISSUE_NUM} -R kai-kou/gem-hunter --json labels \
...
if [ "$NEXT" -lt 3 ]; then
  ...
  gh label create "research-skip:${NEXT}" -R kai-kou/gem-hunter --color FBCA04 2>/dev/null || true
  [ "$SKIP" -gt 0 ] && gh issue edit {ISSUE_NUM} -R kai-kou/gem-hunter --remove-label "research-skip:${SKIP}"
