...
| **陳腐化** | 本文が言及するツール・API・仕様が現状と一致するか（L-114: gh/repo REST の可否記述など、実測が覆っているのに古い結論を残していないか）を `docs/rules/lessons-core.md` Warm 層索引と突き合わせる。旧世代モデル向け表現の検出には `claude-api` スキル（bundled・v2.1.221〜）の `prompt-audit` サブコマンドが使える（#409） |
...
`self-improvement-loop` の発見モードや週次ゲート（プロジェクトの定期ルーティンに組み込む）から手動 or 定期実行する。
