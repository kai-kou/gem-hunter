...
- 🔴 **本番デプロイ（`npm run deploy`）の発火点は PR 種別で分岐する**（`sprint-env-lifecycle-20260820`
  ...
  - **非スプリント PR**（改善 Issue・retro-try・docs 等）: `python3 tools/check_deploy_gate.py` を実行し、
    **終了コード 0（デプロイ可）のときだけ** デプロイする。1（未確定/`rejected` のスプリント Issue が
    main 上に残っている＝待機）/ 2（判定不能）は **デプロイしない**（fail-closed）。待機・判定不能の
    理由はコマンド出力をそのまま PR/Issue コメントに記録する（次に該当 PR が再チェックされるまで
    デプロイは保留のまま）。
