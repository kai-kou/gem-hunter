...
| **5** | `status:waiting-claude` の Issue のうち、タイトルが `SP-n` 規約（`^SP-(\d+):`）に一致しないものが存在する（**`type` で絞らない**。ただし `release:deferred` は除く＝提出後に回すと決めたもの・`docs/05_release/pre-release-gate.md` §3） | バックログ消化（既定 5 件/回。本ルーティンでは firing の残り予算次第で件数を絞ってよい）。🔴 **提出前ゲートが稼働中は `docs/05_release/pre-release-gate.md` §5 の上書き（対象スコープ・選択順・件数上限）を読み、`release:required` を指定順に 1 件だけ処理する**（上書きが許される根拠は `self-improvement-loop` SKILL.md 消化モード冒頭。ゲート終了後は本行の参照ごと撤去する） | `self-improvement-loop` 消化モード |
