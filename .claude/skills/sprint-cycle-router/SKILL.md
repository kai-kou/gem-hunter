...
| **2**   | `check_pending_pr_reviews.py --mine --actionable-only`（相当）が非空。🔴 **`status:blocked` ラベルが付いた PR は対象から除外する**（`--actionable-only` が `blocked_circuit_breaker` ステータスとして機械的に除外する・#746。A-4 サーキットブレーカー発動済みの PR を次 firing が拾い直すと A-4 の目的＝無限修正ループの防止が無効化されるため）                                                                                                                                                | レビュー対応・自動マージ・公開反映まで継続。新規スプリント着手より優先（CP-4: 中途 PR を放置して新規に手を広げない）                                                                                                                                                                                                                                                                                                                                       | `pr-review-watcher`                                                              |
