...
5. 2（`check_prod_drift.py` 自体が判定不能）→ 🔴 **「判定不能」で止めず、根本原因まで踏み込む**（#694）:
   5-1. まず 2 で実行した `check_prod_drift.py --json` の出力を見る。`"trigger_not_configured": true`
        が載っていれば、その時点で **trigger が 0 件** と確定している（#693 で両スクリプトの
        フィールド名を揃えた）。載っていなければ
        `python3 tools/trigger_workers_build.py --dry-run --json --skip-gate-check` を実行し、
        build trigger を実際に解決できるか確かめる（`--dry-run` は POST を止める安全な確認モード）。
        どちらかの出力 JSON に `"trigger_not_configured": true` があれば **trigger が 0 件**、すなわち
   ...
   5-2. `[prod-drift]` の open Issue（無ければ 4 と同じ重複チェックを経て 1 件だけ起票）へ、
        **機械可読マーカーで始まる 1 行** を含む追記コメントを書く。文言ではなくこのマーカーで
        ...
        🔴 **マーカーはコメントの先頭行に置く**。判定は行頭一致で行うため、過去のコメントを
        引用（`> [prod-drift][解消] …`）したり本文中で言及したりしても回数には数えない
        （引用だけで連続カウントが途切れ、通知が飛ばなくなる事故を防ぐ）。
   5-3. そのコメント履歴を `mcp__github__issue_read(method="get_comments")` で取得して JSON 保存し、
        ...
        保存する JSON は **API 応答をそのまま**（各要素の `author_association` を落とさずに）書く。
        ...
        - `escalate: true` → `python3 tools/slack_notify.py` で通知する。通知タイプは必ず `waiting`
          ...
          立てるため、以降の再発が抑制側へ倒れる）。先頭行の書式は **`[prod-drift][通知済み]` の
          直後に対象の症状マーカーを併記する**（直後に置かないと抑制が効かない。判定は
          `[prod-drift][通知済み]` の次に来るトークンだけを見る）:

          ```
          [prod-drift][通知済み] {症状マーカー} について Slack 通知した（区分: {A-6 または 通常}, ts=...）
          ```

          `[prod-drift][経路未構成]` は A-6（`@mention` 付き）、`[prod-drift][判定不能]` は
          A 区分ではない通常通知なので、**区分をそのまま転記しない**（コメント履歴が唯一の監査証跡で、
          誤った区分が残ると後続の firing と人手のレビューが A 区分/B 区分を読み違える）。
...
`[publish-sync]` Issue と同じ作法。[#407] publish-sync は非採用（SSOT: `docs/rules/pr-review-flow-summary.md`）で、
ここでは **Issue 命名と重複起票防止の作法** だけを引き継ぐ）。
...
b) 自分の in-progress Issue / open PR の存在チェック（`check_pending_pr_reviews.py --mine-or-automation --actionable-only --record-approx-sample`
   相当・1 クエリ。§1 のチャネルに応じて MCP/gh/curl 手動実装のいずれかで実行。`--record-approx-sample` は
   無人 firing で REST 近似が常用パスになる実績を溜めるため必ず付ける・Issue #806）。
   **Step 2 と同じ条件に揃える**（#870）: 自 PR があればそれだけ、無ければ bot 自動化 PR
   （Dependabot / `automation/gem-pool-refresh`・`D-43`）も対象になる。**他者の人手 PR は対象外**
...
| **2**   | `check_pending_pr_reviews.py --mine-or-automation --actionable-only --record-approx-sample`（相当。`--record-approx-sample` は無人 firing の近似判定実績を溜めるため必ず付ける・Issue #806）が非空。**選択の順序は自スコープ優先**（#47 / #870）: 自 PR（`Session-Id:` 一致）が 1 件でもあればそれだけを対象にし、0 件のときだけ bot 自動化 PR（Dependabot / `automation/gem-pool-refresh`・`D-43`）を対象にする。bot PR は PR 本文に自セッションの `Session-Id:` を持たないため、旧条文の `--mine` 単独では **構造的に決して拾えなかった**（#870）。🔴 **他者の人手 PR は Step 2 では拾わない**（CP-4・L-109 の不介入。孤児 PR の救済は Step 2 の責務ではない）。**回収は Step 6（衛生スロット）の `project-sync` Step 3.5 が Orphan PR（最終更新 24 時間超）として担う**（下記「人手孤児 PR はどのブランチが回収するか」・#898）。🔴 **`status:blocked` ラベルが付いた PR は対象から除外する**（`--actionable-only` が `blocked_circuit_breaker` ステータスとして機械的に除外する・#746。A-4 サーキットブレーカー発動済みの PR を次 firing が拾い直すと A-4 の目的＝無限修正ループの防止が無効化されるため） | レビュー対応・自動マージ・公開反映まで継続。新規スプリント着手より優先（CP-4: 中途 PR を放置して新規に手を広げない）。🔴 **bot PR を選んだ場合の排他はレイヤー 5（`active_session` の 10 分窓）だけ**（#870）: 自 PR と違い `Session-Id:` による決定論的所有判定（レイヤー 6）が無く、2 firing が数分差で同じ bot PR を選びうる。**マージ直前に `active_session` を再確認してから `merge_pull_request` を呼ぶ**（`session-concurrency-rules.md` レイヤー 3「PR 作成前の再チェック」と同型の再チェック）。ラベル付与による論理ロックの新設はしない（設計判断のため別 Issue） | `pr-review-watcher`                                                              |
...
| **6**   | 当日の衛生スロット未実施（`project-sync` ログなし）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | 監査・衛生。**他セッションが作成した人手孤児 PR（最終更新 24 時間超）の回収を含む**（`project-sync` Step 3.5・#898）                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `workflow-health-check` 軽量版 → `project-sync`                                  |
...
### 人手孤児 PR はどのブランチが回収するか（#898）

Step 2 が `--mine-or-automation` 限定なのは、CP-4・L-109 の **他セッション対応中 PR への不介入** を守るため
（`active_session` の 10 分窓を過ぎても、他セッションが思考中断から戻って続きを進めるケースが実在する）。
したがって `is_mine: false` かつ `active_session: false` の **他者の人手 PR は Step 2 では拾わない**。

- **回収先は Step 6 → `project-sync` Step 3.5（Orphan PR・最終更新 24 時間超）**。そこから
  `pr-review-watcher` フローへ復帰して指摘対応・マージまで進める（CP-3「Issue / PR ゼロ放置」を満たす）
- 🔴 **回収まで最大 24 時間かかるのは意図的に受け入れたコスト** であり、放置を許容しているのではない。
  不介入（CP-4）と非放置（CP-3）を両立させる境界として 24 時間を置いている
- 🔴 **`pr-review-flow-summary.md`「セッション復帰（PR 放置検出）」② の全体ビュー**
  （`--actionable-only` 単独・孤児 PR 救済）は **対話セッションの復帰時に人間の判断で使う経路** であり、
  無人 firing の決定木 Step 2 とは適用場面が違う。**Step 2 で ② を実行しない**（片方の文書だけを読んで
  Step 2 の対象を拡大解釈しない）
- 24 時間未満の人手 PR を発見しても、この firing は Step 2 として拾わない。緊急性があると判断したなら、
  **拾うのではなくその事実を Issue に記録する**（判断の即興ではなく次回の条文改定に回す）
...
4-3. Sprint Planning コメントを投稿する。**編成の既定（単独実行禁止・sp 別の役割数）の正本は
     ...
     **スプリント着手時に `解消する Issue` 欄を追加記載する**: SP-n が別 Issue を解消対象に掲げている場合、
     各 Issue 番号と対応する完了条件を列挙する（例: `#173 は condition 1-3、#179 は condition 1-2` 等）。
     完了条件の転記により「機能要件は満たしたがドキュメント更新等の完了条件は満たされていない」という
     部分達成を着手時に可視化し、スプリントレビューでの後追い発覚を減らす。
...
| A-4 サーキットブレーカー発動（修正サイクル 2 回超） | 該当箇所での修正試行回数                                                                   | 該当 Issue/PR に `status:blocked` を付与。**Step 2 / Step 3 / Step 4 / Step 5 の対象クエリから除外**（`status:blocked` を除外条件に必ず含める・`self-improvement-loop` の除外リストと同じ書式）。**Step 2 の除外は `tools/check_pending_pr_reviews.py` の `_label_based_early_exit_status()` / `ACTIONABLE_EXCLUDED_STATUSES`（`blocked_circuit_breaker`）が機械実装（#746。旧仕様は `status:waiting-user` しか判定しておらず `status:blocked` は未判定だった＝PR #735 が Step 2 に拾い直された実際の原因）。waiting-user 通知は `user-notification-triage.md` の A 区分基準どおり必要時のみ |
...
- [ ] `status:blocked` の Issue/PR が Step 2 / Step 3 / Step 4 / Step 5 の対象から除外されている（Step 2 は `check_pending_pr_reviews.py` の `blocked_circuit_breaker` 判定で機械固定・#746）
- [ ] Step 5 で `SP-n` 以外の `status:waiting-claude` Issue を `type` で取りこぼしていない
- [ ] 他者の人手 PR を Step 2 で拾っていない（回収は Step 6 の `project-sync` Step 3.5・#898）
