...
3. 失敗 → `curl -o /dev/null -w '%{http_code}' -H "Authorization: Bearer $GH_TOKEN" \
   ...
   かつ著者が `dependabot[bot]` / `github-actions[bot]` かつ fork でない PR を対象にする。
   **他者の人手 PR は対象にしない**（CP-4・L-109）。
