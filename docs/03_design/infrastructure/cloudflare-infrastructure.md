...
| `INF-2` 定常コストをゼロに | Free plan を維持する限り超過は課金ではなく停止（§5） | ❌ **実測により不成立が確定**（本番 p95 CPU 時間が Free の上限を大幅に超過。実測値の正本は §5.3 →参照先 [`roadmap.md`](../../02_requirements/roadmap.md) §3 `M-1` の `D-17`。§5 のとおり本アカウントは Paid で運用） |
...
- 🔴 **値は `SP-18`（#388）で 50 → 200 へ引き上げた**（**引き上げ判断の正本** は `D-38` の `SP-18` 実測追記 / **p95 実測値の正本** は本項末尾のリンク先）。理由は `D-38` のシャード配信で cold start の CPU が **約 81ms**（`JSON.parse` 39.9ms + `Map` 構築 40.7ms・ローカル実測）に達し、暫定 50ms では超過しうるため。🔴 **継続的に** 超過すると **Error 1102**（`Worker exceeded resource limits` / invocation outcome `exceededCpu`）になる（1027 は Free の使用量到達による停止で Paid では出ない）。⚠️ **1 回超えただけでは打ち切られない**: isolate には設定値を散発的に超える場合の組み込みの許容があり、公式は「継続的に（consistently）超え始めたときに打ち切る」と定めている（[Wrangler 設定の Limits 節](https://developers.cloudflare.com/workers/wrangler/configuration/)）。したがって **`exceededCpu` がゼロであることは「上限内に収まっている」証拠にならない**（**#782 の実測**: `cpu_ms = 400` に対して 496 ms のリクエストが 200 で完了した。**原因の解明は #860**）。上限の妥当性は outcome ではなく `cpuTime` の分布で判断する。🔵 **Workers 上の p95 は実測済み**（#782・2026-09-03 JST・p95 = 320 ms。実測値の正本は [`roadmap.md`](../../02_requirements/roadmap.md) §3 `M-1` の `D-17`）。軽量アセット化（Issue #434）で締め直せる見込みがある
- 🔴 **さらに `SP-19`（#389）で 200 → 400 へ引き上げた**（`D-38` の `SP-19` 実測追記が正本）。Gem 一覧は所属判定用の `Map` だけでなく **全レコード + 照合用トークン列** を同じ 1 回の parse から作るため、**一覧の初回リクエストが 237〜277ms**（ローカル実測・別プロセス 5 回）に達し 200ms では **継続的な超過時に Error 1102** になる。🔵 一覧専用のコスト（トークン計算 約 91ms + 並べ替え 約 31ms）は初回の一覧リクエストまで遅延させてあり、`SP-18` で出荷済みのバッジ経路（`lookup` のみ）は **96〜110ms**（`SP-18` 実装の同条件 84〜90ms に対し +約 14ms）に留まる。🔴 **400 にした理由**: 観測した最大は 277ms で、300 では余裕が約 8% しか残らない。**`SP-19` の判断時点では** Workers 実機の p95 が未測定だった（`wrangler tail` がプロキシを通らない）ため、そこが枯れる保証がないとして **実測最大に対して約 44% の余裕** を取り Error 1102 を潰す側に倒した。🔵 **その後 #782 で実測済み**（2026-09-03 JST・p95 = 320 ms）。🔵 `limits.cpu_ms` は **上限のキャップであって予約ではない**（課金は実使用 CPU に対して起きる）ので、上げても平常時のコストは増えない
...
> 🔵 `D-19` が約束した Billable Usage API の日次ポーリングは **実装・実データでの疎通確認とも完了**（`tools/check_cloudflare_cost.py`・Issue #1016 で Billing Read 権限を付与、Issue #939 で応答形式を実測・確定）。
> ⚠️ **既知の限界は残る**（同一日内の一部内訳行だけが欠落するケースは検知できない）。詳細は §5.5。
...
✅ **計測 1（p95 CPU 時間）は実測完了・判定確定**（Issue #782・2026-09-03 JST）: 本番 `wrangler tail --format json` で実測した p95 は上記の閾値（7〜8 ms）および Free の上限 10 ms（§5.2）を大幅に超過しており、**Free では INF-3 を満たせないことが確定した**。本アカウントは Paid のまま継続する（§5.1）。実測値の生データ・p95 の算出方法・単位が ms であることの根拠は、この節へ複製せず [`roadmap.md`](../../02_requirements/roadmap.md) §3 `M-1` の `D-17` 項目を正本として参照する。
...
| Billable Usage API（日次・§5.5） | 製品別コストの事後取得 | 日次ポーリングに起因する検知ラグ / **日単位の粒度**（同一日内の一部内訳行の欠落は検知できない） |
| Budget alerts | 閾値到達の通知 | 🔴 **停止しない**（公式に "informational only" と明記） |

🔴 **残余リスク**: Paid 移行後は、大量アクセスによるリクエスト数課金の急増を止める native な手段が存在しない。Billable Usage API のポーリングによる後追い封じ込めしか残らない。⚠️ **そのポーリングは実データでの疎通・検知ロジックとも稼働している**（§5.5）が、**日単位の検知に留まる**（同一日内の一部内訳行だけが欠落するケースは検出できない）ため、残余リスクは低減されているが完全には解消されていない。

🟢 **撤退ラインは月額 $10**（`D-19`・ユーザー決定）。Billable Usage API（`GET /accounts/{id}/billable-usage`）の日次ポーリングでこの閾値を監視する。⚠️ Budget alerts は通知のみで停止しないため、これに依存しない。監視の実装・実行経路・現時点で残る制約は §5.5 が正本（本節では繰り返さない）。

### 5.5. コスト監視の実装（`tools/check_cloudflare_cost.py`・Issue #247）

> 🔵 **本節がコスト監視ツールの仕様の正本**（ツール名・オプション・終了コードの意味を書くのはここだけ。他節・他ドキュメントからは本節を参照する）。

`GET /accounts/{account_id}/billable-usage` を取得し、次の 2 軸で判定する。

| 軸 | 判定内容 | 既定値 |
|---|---|---|
| 累計 | 当月（JST 基準）の課金対象累計が撤退ライン（`D-19`）を超えたか | `--threshold-usd 10.0` |
| 急増 | 直近日が前日比で何倍に跳ねたか | `--surge-ratio 3.0` |

🔴 **急増判定には絶対額の床がある**（ここに書いていないと「`--surge-ratio` を下げたのに検知しない＝壊れている」と誤診する）。

- **直近日が `SURGE_MIN_ABS_USD = 1.0`（$1.00）以上のときだけ倍率を評価する。** 直近日が $1.00 未満なら、たとえば $0.20 → $0.80（4 倍）は `--surge-ratio 2.0` へ下げても検知されない。
- **前日が $0.00 のときは倍率を算出できない**（ゼロ除算になる）ため、倍率は出さずに「直近日が `SURGE_MIN_ABS_USD`（$1.00）以上なら急増として扱う」特例で判定する。

**終了コード**

| コード | 意味 |
|---|---|
| `0` | ① 判定した結果が閾値内（累計が撤退ライン **以下**。ちょうど $10.00 は超過ではない）かつ急増なし、**または** ② `--gate-daily` が当日実行済みでスキップした（＝この実行では評価していない） |
| `1` | 閾値超過または急増を検知 |
| `2` | 判定不能（**fail-closed**。API 到達不能・権限不足・応答形式の不一致など） |

⚠️ **`0` は「評価して正常だった」と「評価していない（当日スキップ）」の両方を含む。** 終了コードだけで分岐する下流は、スキップ日を「監視は緑」と読み違える。厳密に扱う下流は `--json` の `skipped` フィールドを見る。

**主なオプション**: `--json`（機械可読出力。`--gate-daily` のスキップ時も出力し `skipped: true` を含む）/ `--gate-daily`（JST 当日 1 回だけ実行するゲート・Stop hook 用。当日実行済みならスキップし終了コード `0` を返す）/ `--dry-run` / `--self-test` / `--threshold-usd` / `--surge-ratio`。

**実行経路**

| 経路 | 呼び方 | 目的 |
|---|---|---|
| `.claude/hooks/stop-slack-notify.sh` | `--gate-daily` | JST 当日 1 回の日次ポーリング（本判定） |
| `tools/run_checks.sh` | `--self-test` のみ | 判定ロジックの回帰検査（外部 API 疎通に依存する本判定は PR ゲートに載せない・`check_prod_drift.py` と同じ扱い） |

⚠️ **日次 1 回へ収束するのは「判定に成功した日」だけ**（当日マーカーは判定できたときにしか打たない）。判定不能（終了コード `2`）の日は毎回の Stop で再試行する（Billing Read 権限は付与済み・§5.5「実態」のため、この再試行は API 側の一時的な障害・レート制限に対する防御であり恒久状態ではない）。

Issue の起票はツール側では行わない。超過内容を 1 行で出力し、通知は `tools/triage_notification.py` の **A 区分（`A-6`）** として扱う。

🔴 **実額を含む文面の転記先は Slack（非公開）に限定する。** 本リポジトリは公開前提（[`repository-publication-review.md`](../../05_release/repository-publication-review.md)）で、超過アラートの 1 行には月内累計・直近日・前日の **実課金額** が入る。公開リポジトリの Issue / PR コメントへ書く場合は金額を伏せた要約（例「撤退ライン超過（実額は Slack 通知参照）」）を使う。

#### 実態（2026-09-08 JST 時点・Issue #939 解消）

- **Billing 系の Read 権限は付与済み**（Issue #1016・飼い主が 2026-09-08 JST に `gem-hunter-deploy` トークンへ `Account` → `Billing` → `Read` を追加）。**実 API での疎通確認も完了しており**、実行すると正常に判定できる（exit `0`）ことを確認済み。
- **応答形式は実測して確定した**（PR #1101・`c1f9a29`）。候補集合で日付キー・金額キーを拾う実装は廃止し、実測キーへ固定した。`GET /accounts/{account_id}/billable-usage` の応答は FOCUS 仕様（FinOps Open Cost and Usage Specification）の 25 フィールドで構成される。

  | 項目 | 実測結果 |
  |---|---|
  | 日付キー | `ChargePeriodStart`（日次） |
  | 金額キー | `BilledCost`（実際に請求される額。`EffectiveCost` / `ListCost` / `ContractedCost` は割引前後・契約価格の値のため撤退ラインの判定には使わない） |
  | 通貨 | 別フィールド `BillingCurrency`（値 `USD`）。キー名自体に通貨は含まれない |
  | 請求期間開始日 | `BillingPeriodStart`（全レコードで同一値。anchor day は毎月 28 日 — `GET /accounts/{id}/billable-usage/info` の `billing_cycle_anchor_timestamp` で確認） |
  | ページング | 無し。`result_info` は常に `null`、`page` / `per_page` は無視される（**実測範囲: 139 件までは 1 回の応答で全件返ることを確認済み。それを超える件数での上限の有無は未確認**） |
  | `from` / `to` | 日付フィルタとして機能するが、範囲が anchor day を含まないと 0 件になる（実測: `from=2026-09-01&to=2026-09-07` → 0 件） |
  | データの粒度 | 1 日あたり 11〜13 件（サービス別の内訳）。**最終日だけ件数が少ない**（集計途中。翌日には満額になる） |
  | v2 エンドポイント | `GET /accounts/{id}/billable/usage` は `code 1171: insufficient_permissions`（Cloudflare 側の Restricted 指定・権限追加では解決しない） |
  | レート制限 | 連続アクセスで `code 971 "Please wait and consider throttling"` を返す。疎通確認は間隔を空けて 1 回ずつ行う |

- **実装が持つ検知ロジック**（PR #1101）: ① 全レコードの `BillingCurrency` が `USD` であることを検証（1 件でも違えば fail-closed）② `BillingPeriodStart` から最終レコード日まで日付が 1 日も欠けていないか検証（欠落があれば fail-closed）③ 応答が末尾で丸ごと打ち切られたケースを検知（最終日の陳腐化と請求期間開始からの経過日数の AND 判定・`FRESHNESS_MAX_LAG_DAYS = 2`）④ 集計途中で過少になる最終日を前日比の急増判定から除外する（月内累計には含める）。

🔴 **既知の限界（未解消・意図的に実装を見送っている）**: 上記の検知はいずれも **日単位**。**同一日内の一部の内訳行だけが欠落するケース（日付自体は生存している）は検出できない**。件数（1 日あたり想定 11〜13 件からの逸脱）をヒューリスティックとして使う案は誤検知リスクが高いため見送った（Layer 1 セルフレビューで PLAUSIBLE 指摘・`tools/check_cloudflare_cost.py` の docstring にも残存リスクとして記録済み）。

- 権限付与は `A-6`（ユーザーのアカウント権限が物理的に必要）だった。発行先は https://dash.cloudflare.com/profile/api-tokens 、必要権限は `Account` → `Billing` → `Read`（§11.2 の `H-6`）。
  - 🔴 **既定は既存 `CLOUDFLARE_API_TOKEN` への権限追加**（飼い主決定・Issue #1068）: デプロイ権限を持つ既存トークン（GitHub Actions Secrets とセッション env の 2 箇所に配置済み）へ `Account` → `Billing` → `Read` を追加した。**専用トークン `CLOUDFLARE_BILLING_API_TOKEN` の新規発行は推奨から外している**（従来はこちらを推奨していた）。
  - **決定理由**: トークンが増えると運用者（人間）が管理対象を失念するリスクがあり、その運用リスクが最小権限による配布面縮小の利得を上回ると判断した（Issue #1068）。🔴 **この決定記録は Issue #939（応答形式の実測）の解消とは独立である。** 本節を整理した後もこの決定理由とトレードオフの 2 行は残す（判断根拠を失うと同じ論点を再度ゼロから議論することになる）。
  - **トレードオフ**: 配置箇所（GitHub Actions Secrets とセッション env の 2 箇所）は増えないが、**そのトークン自体の権限が広がる**。漏洩時には Workers のデプロイと Billing 情報の読み取りが同一トークンで可能になるため、ローテーションの優先度は従来より高い。最小権限を一部妥協している。
  - 実装は `CLOUDFLARE_BILLING_API_TOKEN` があればそれを優先し、無ければ `CLOUDFLARE_API_TOKEN` へフォールバックする（この経路自体は変更していない）。将来トークンを分離したくなった場合はこのフォールバック経路が有効な移行先になる。
- 🔵 **運用メモ**: API トークンの ID から、ダッシュボードの該当トークン画面を直接開ける URL 形式は `https://dash.cloudflare.com/profile/api-tokens/{token_id}`（公式ドキュメント未記載・実測で有効性を確認済み）。トークン ID は `GET /user/tokens/verify` で取得できる（この呼び出しに追加権限は不要）。複数トークンが存在するとき、どれがセッションの `CLOUDFLARE_API_TOKEN` かを特定するのに使える。
...
🔵 **H-1 〜 H-3 が済めば、以降のリソース作成・デプロイ・プレビュー URL 生成・シークレット投入・トークンのローテーション発行はすべて Claude が非対話で実行できる。** ⚠️ **例外**: コスト監視の実データ疎通には Billing Read 権限が別途必要だった。🟢 **付与済み**（Issue #1016・2026-09-08 JST。§11.2 の `H-6`・§5.5）。
...
| **H-6** | API トークンへ `Account` → `Billing` → `Read` を付与する（既定は既存 `CLOUDFLARE_API_TOKEN` への権限追加・Issue #1068） | 🟢 **対応済み**（Issue #1016・2026-09-08 JST に付与。応答形式の実測と実装追随は Issue #939 / PR #1101 で完了）。手順と最小権限の方針、実測仕様と既知の限界は §5.5 が正本 |
...
| 2 | ~~RSC レンダリングの p95 CPU が 10 ms に収まるか~~ | ✅ **解消済み**（Issue #782・2026-09-03 JST）: 本番 `wrangler tail --format json` 実測で Free 上限 10 ms を大幅に超過し **Paid が必須** であることを実測で確定した。実測値（生データ・p95 の値・算出根拠）の正本は [`roadmap.md`](../../02_requirements/roadmap.md) §3 `M-1` の `D-17` 項目（本表へは数値を複製しない） | 解消済み |
...
| 10 | ~~`GET /accounts/{id}/billable-usage` の応答形式（日付キー・金額キーの実名）が未実測で、候補集合から拾っている~~ | ✅ **解消済み**（Issue #939・PR #1101・2026-09-08 JST）: Billing Read 権限付与（Issue #1016）後に実 API へ疎通し、応答が FOCUS 仕様の 25 フィールドであること・日付キー `ChargePeriodStart` / 金額キー `BilledCost` / 通貨キー `BillingCurrency`（`USD`）を実測で確定した。候補集合による推測は廃止し実測キーへ固定済み。実測値・検知ロジックの詳細は §5.5 が正本 | 解消済み |
