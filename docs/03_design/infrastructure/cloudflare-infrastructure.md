...
> 🔵 `D-19` が約束した Billable Usage API の日次ポーリングは **判定ロジックのみ実装済み**（`tools/check_cloudflare_cost.py`）。
> **実データでの疎通は未確認**（現行トークンに Billing Read 権限が無いため常に「判定不能」へ倒れる）。詳細は §5.5。
...
| Billable Usage API（日次・§5.5） | 製品別コストの事後取得 | バースト検知のラグ / **現状は権限不足で値が取れない** |
| Budget alerts | 閾値到達の通知 | 🔴 **停止しない**（公式に "informational only" と明記） |

🔴 **残余リスク**: Paid 移行後は、大量アクセスによるリクエスト数課金の急増を止める native な手段が存在しない。Billable Usage API のポーリングによる後追い封じ込めしか残らない。⚠️ **そのポーリングも現時点では実データを取得できていない**（§5.5 の「実態として残る制約」）ため、**残余リスクは未低減のまま** である。
...
⚠️ **日次 1 回へ収束するのは「判定に成功した日」だけ**（当日マーカーは判定できたときにしか打たない）。判定不能（終了コード `2`）の日は毎回の Stop で再試行する。Billing Read 権限が未付与の間はこれが恒久状態になるため、遅延を抑えたいときは疎通確認（Issue #939）まで Stop hook 側の呼び出しを外す。
...
#### 🔴 実態として残る制約（2026-09-05 JST 時点）

- 現行の `CLOUDFLARE_API_TOKEN` には **Billing 系の Read 権限が無い**。したがって **実 API での疎通確認はできておらず**、実行すると終了コード `2`（判定不能）へ倒れる。
- **応答形式は未実測**。日付キー・金額キーを候補集合で拾っているため、**疎通確認時に実測キーへ絞り込むまでは金額の取り違えリスクが残る**（§12 の未確認事項 10）。
- **担保できているのは判定ロジック・終了コード経路・フェイルセーフの挙動のみ**（`--self-test` と変異テストで固定済み）。「監視が効いている」状態ではない。
- 権限付与は `A-6`（ユーザーのアカウント権限が物理的に必要）。発行先は https://dash.cloudflare.com/profile/api-tokens 、必要権限は `Account` → `Billing` → `Read`（§11.2 の `H-6`）。
  - 🔴 **既定は既存 `CLOUDFLARE_API_TOKEN` への権限追加**（飼い主決定・Issue #1068）: デプロイ権限を持つ既存トークン（GitHub Actions Secrets とセッション env の 2 箇所に配置済み）へ `Account` → `Billing` → `Read` を追加する。**専用トークン `CLOUDFLARE_BILLING_API_TOKEN` の新規発行は推奨から外す**（従来はこちらを推奨していた）。
  - **決定理由**: トークンが増えると運用者（人間）が管理対象を失念するリスクがあり、その運用リスクが最小権限による配布面縮小の利得を上回ると判断した（Issue #1068）。🔴 **この決定記録は Issue #939（応答形式の実測）の解消とは独立である。** 本節を「制約が解消された」という理由で整理するときも、決定理由とトレードオフの 2 行は残す（判断根拠を失うと同じ論点を再度ゼロから議論することになる）。
  ...
  - 実装は `CLOUDFLARE_BILLING_API_TOKEN` があればそれを優先し、無ければ `CLOUDFLARE_API_TOKEN` へフォールバックする（この経路自体は変更しない）。将来トークンを分離したくなった場合はこのフォールバック経路が有効な移行先になる。
- **残作業の追跡先は Issue #939**（疎通確認・応答キーの絞り込み・本節の更新）。付与後に実データで疎通を確認してから本節を更新する。
...
| 10 | `GET /accounts/{id}/billable-usage` の応答形式（日付キー・金額キーの実名）が未実測で、候補集合から拾っている | Billing Read 権限（`H-6`）の付与後、初回実行で実応答を記録し候補集合を 1 つのキーへ絞る（Issue #939） | 中（金額の取り違え） |
