...
| Billable Usage API（日次・§5.5） | 製品別コストの事後取得 | バースト検知のラグ（最大 24 時間）/ **日単位の粒度**（同一日内の一部内訳行の欠落は検知できない） |
...
- **応答形式は実測して確定した**（PR #1101・`c1f9a29`）。候補集合で日付キー・金額キーを拾う実装は廃止し、実測キーへ固定した。`GET /accounts/{account_id}/billable-usage` の応答は FOCUS 仕様（FinOps Open Cost and Usage Specification）の 25 フィールドで構成される。
  ...
  | ページング | 無し。`result_info` は常に `null`、`page` / `per_page` は無視され、請求期間の全レコードが 1 回で返る |
...
🔵 **H-1 〜 H-3 が済めば、以降のリソース作成・デプロイ・プレビュー URL 生成・シークレット投入・トークンのローテーション発行はすべて Claude が非対話で実行できる。** ⚠️ **例外**: コスト監視の実データ疎通には Billing Read 権限が別途必要で、その付与は `A-6` として人間に残る（§11.2 の `H-6`・§5.5）。
...
| **H-6** | API トークンへ `Account` → `Billing` → `Read` を付与する（既定は既存 `CLOUDFLARE_API_TOKEN` への権限追加・Issue #1068） | コスト監視（`tools/check_cloudflare_cost.py`）の実データ疎通を開始するとき・`D-19`。手順と最小権限の方針は §5.5 が正本（追跡は Issue #939） |
