...
| 軸 | 判定内容 | 既定値 |
|---|---|---|
| 累計 | 当月の課金対象累計が撤退ライン（`D-19`）を超えたか | `--threshold-usd 10.0` |
...
| コード | 意味 |
|---|---|
| `0` | 閾値内（累計が撤退ライン未満かつ急増なし） |
| `1` | 閾値超過または急増を検知 |
| `2` | 判定不能（**fail-closed**。API 到達不能・権限不足・応答形式の不一致など） |

**主なオプション**: `--json`（機械可読出力）/ `--gate-daily`（JST 当日 1 回だけ実行するゲート・Stop hook 用）/ `--dry-run` / `--self-test` / `--threshold-usd` / `--surge-ratio`。
...
- 権限付与は `A-6`（ユーザーのアカウント権限が物理的に必要）。発行先は https://dash.cloudflare.com/profile/api-tokens 、必要権限は `Account` → `Billing` → `Read`。付与後に実データで疎通を確認して本節を更新する。
