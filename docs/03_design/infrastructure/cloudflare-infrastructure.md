...
- 権限付与は `A-6`（ユーザーのアカウント権限が物理的に必要）。発行先は https://dash.cloudflare.com/profile/api-tokens 、必要権限は `Account` → `Billing` → `Read`（§11.2 の `H-6`）。
  ...
  - **決定理由**: トークンが増えると運用者（人間）が管理対象を失念するリスクがあり、その運用リスクが最小権限による配布面縮小の利得を上回ると判断した（Issue #1068）。
  - **トレードオフ**: このトークンはデプロイ権限を持ち、GitHub Actions Secrets とセッション env の 2 箇所に配置されているため、コスト参照のためだけに強い権限の配布面が広がる。最小権限を一部妥協している。
