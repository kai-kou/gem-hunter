...
- 権限付与は `A-6`（ユーザーのアカウント権限が物理的に必要）。発行先は https://dash.cloudflare.com/profile/api-tokens 、必要権限は `Account` → `Billing` → `Read`（§11.2 の `H-6`）。
  - 🔵 **推奨は専用トークン**: `Account` → `Billing` → `Read` **だけ** を持つトークンを新規発行し、`CLOUDFLARE_BILLING_API_TOKEN` として登録する。デプロイ権限を持つ既存 `CLOUDFLARE_API_TOKEN`（GitHub Actions Secrets とセッション env の 2 箇所に配置済み）へ権限を足すと、コスト参照のためだけに強い権限の配布面が広がるため最小権限を逸脱する。
  - 実装は `CLOUDFLARE_BILLING_API_TOKEN` があればそれを使い、無ければ `CLOUDFLARE_API_TOKEN` へフォールバックする。既存トークンへの権限追加はこのフォールバック経路として残してよいが、**推奨手順は専用トークンの発行** である。
...
| **H-6** | API トークンへ `Account` → `Billing` → `Read` を付与する（推奨は同権限のみを持つ専用トークン `CLOUDFLARE_BILLING_API_TOKEN` の新規発行） | コスト監視（`tools/check_cloudflare_cost.py`）の実データ疎通を開始するとき・`D-19`。手順と最小権限の方針は §5.5 が正本（追跡は Issue #939） |
