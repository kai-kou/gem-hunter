...
| 作業 | 発生頻度 | 手当て |
|---|---|---|
| ~~PAT の有効期限更新~~ | ~~期限依存~~ | 🟢 **`D-20`（GitHub App 方式・[ADR 0003](../../adr/0003-github-app-authentication.md)）で解消済み**。有効期限を持つのは 1 時間で自動失効する installation token であり、アプリが取り直すため人手の更新作業が発生しない。人が保持するのは期限のない署名鍵だけになる |
