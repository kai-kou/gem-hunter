...
| **L2** | **アプリ内 `CachePort` の実装**（サーバー側の共有キャッシュ） | 事業者依存（インスタンス / 事業者のキャッシュ基盤） | ✅ 採用。MVP の主役。⚠️ **フレームワーク（Next.js）のデータキャッシュには担保を負わせない**（下記注記） |
...
2. キャッシュヒット率が想定を下回り、`INF-2`（コスト）または `NFR-7`（レート制限耐性）を満たせない
   - 🔴 **「想定」の数値定義（計測条件・想定値・充足判定）は [ADR 0016](../../adr/0016-cloudflare-cache-api-for-cross-isolate-cache.md) §3 が正本**（数値をここに複製しない）。⚠️ **本条件の充足は「L3 を入れる」ではなく「検討して ADR を起票する」を要求する**。実際に Issue #121 では条件 2 を満たしたうえで、検討の結果 L3 ではなく **L2 の実装差し替え** を選んだ（同 ADR §2.3 / §3.3）
...
| ID | 契約項目 |
|---|---|
| **INF-20** | デプロイのトリガーは **git push / マージのみ**。手作業のコマンド実行を運用手順に含めない。⚠️ **暫定運用**: **デプロイ発火点の Workers Builds への移行（`D-31` / `D-32`）が完全に回りきるまでの間、および完了後も手動フォールバックとして**、セッション（Claude）による `wrangler` 直接実行を許容する（`D-23` の緩和を継続。プレビュー URL の取得とデプロイゲート通過後の再デプロイに必要）。🔴 **復帰先は GitHub Actions ではない**（`D-31`・2026-08-21）: Workers Builds（Cloudflare native の Git 連携）へ移行することで原則（push / マージのみがトリガー）へ復帰する。**制限が解除されても GitHub Actions へは戻さない**（移行手順は [`cloudflare-infrastructure.md`](../../03_design/infrastructure/cloudflare-infrastructure.md) §8.2.3）。🔵 **`D-42`（2026-08-24）は品質チェックの Actions 化のみを決めたものであり、本行の緩和条件には影響しない** |
