...
> 🔵 **追記（Issue #167・2026-09-04・一覧→詳細の遷移中）**: 上表の 4 状態は **一覧そのものの状態** を表す。これとは別レイヤーとして、**一覧から詳細への遷移中**（`<Link>` のクライアント遷移が完了するまでの窓）は **リンク内に視覚ヒントを出す**（`LinkPendingHint`・`src/ui/link-pending-hint.tsx`）。一覧の 4 状態を書き換えるわけではないので、読み込み中表示の二重表現にはならない。
>
> - 🔴 **`aria-hidden="true"` で DOM に常設し、`opacity` だけを切り替える**（`display` で出し入れするとクリックのたびにリンク行の幅が変わり、レイアウトシフトを起こす・§8.3）
> - 🔴 **支援技術への通知はリンクごとに増やさない**。一覧ごとに 1 個だけ常設するライブリージョン（`LinkPendingAnnouncer`・`src/ui/link-pending-announcer.tsx`）へ集約し、遷移中だけ文言（`common.linkPending`）を入れる（§7.2）
> - 遷移先の詳細ページに `loading.tsx` を置く解法は取れない（`AC-5` の同期 404 判定を壊すため。理由は `app/[locale]/repos/[owner]/[repo]/page.tsx` 冒頭のコメントが正本）
> - 遷移そのものの設計方針は §8.5、ライブリージョンの規約は §7.2 を参照
...
- 🔴 **リスト項目ごとに増やさない。** 一覧の各行・各リンクの状態通知は、**その一覧に 1 個だけ常設した** ライブリージョンへ集約する（例: 一覧→詳細の遷移通知は一覧ごとに `LinkPendingAnnouncer` 1 個。リンクごとに `role="status"` を置くと一覧 1 ページで数十個のライブリージョンが並ぶ・§4.4）
...
> 🔵 **README のような可変長コンテンツでは「実データと同一寸法」を原理的に達成できない**（実サイズが数行〜数百行まで暴れるため）。この場合は妥協ラインとして **`min-height` を与えるに留め、完全一致は狙わない**。出典: `content/discussions/feedback334_detail_readme_20260821/whiteboard.md` round 2（`readme_render` の指摘）。同じ指摘がレビューで再発するのを防ぐための注記。
...
🔵 **遷移中のペンディング表示は別レイヤー**（§4.4 の追記・Issue #167）: プリフェッチ・スクロール復元といった `<Link>` の標準挙動は変えず、その内側に `LinkPendingHint` を足すだけである。🔴 **本節の「標準挙動に任せる」を根拠に `LinkPendingHint` / `LinkPendingAnnouncer` を削除しない。**
