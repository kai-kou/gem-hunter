...
- **1 ページ合計 100KB・個別 30KB** を目安予算とする。実測値（`ls -l public/images/`・2026-08-22・Issue #359 で `hero-idle` を 16:9 化し `loading` を追加した後の値）は以下のとおりで、**アセットごとに予算消費率が大きく異なる**。

  | アセット | 表示寸法 | 実測サイズ | 個別予算（30KB）比 |
  |---|---|---|---|
  | `logo.webp`（96px 生成） | 24×24 | 2,838 バイト（約 2.8KB） | 9% |
  | `not-found.webp`（320px 生成） | 160px | 9,966 バイト（約 9.7KB） | 33% |
  | `empty-result.webp`（256px 生成） | 96〜120px | 10,400 バイト（約 10.2KB） | 34% |
  | `loading.webp`（256px 生成） | 64×64 | 19,358 バイト（約 18.9KB） | 63% |
  | `hero-idle.webp`（768×432・16:9 へ変換） | 最大幅 320px（16:9） | 27,594 バイト（約 27.0KB） | **92%** |
  ...
  🔴 **「約 10.2KB」は 256px の静止画アセット（`empty-result` 相当）の実測値であり、全アセットに当てはまるわけではない**（同じ 256px 生成でも `loading.webp` は約 18.9KB とばらつく）。特に `hero-idle.webp`（16:9 化後も）は 27.0KB で **個別予算 30KB の約 92% を消費** しており、余裕があるとは言えない。1 ページで同時に読み込むのはロゴ + 状態イラスト 1 枚（例: logo + hero-idle ≒ 30.4KB）で、合計予算 100KB は下回る
