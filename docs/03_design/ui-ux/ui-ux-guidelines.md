...
担保するのは **`e2e/a11y-text-spacing.spec.ts`**。上記 4 値を `page.addStyleTag` で強制注入したうえで、320px 幅（§7.5 と同じく SC 1.4.10 が名指しする最小幅）で ① 横スクロールが発生しない ② カード同士が縦方向に重ならない ③ 主要なテキスト要素が引き続き可視、の 3 点を実測する。
