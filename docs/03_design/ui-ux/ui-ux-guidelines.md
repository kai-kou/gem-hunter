...
- **1 ページ合計 100KB・個別 30KB** を目安予算とする。実測値（`ls -l public/images/`・2026-08-21）は以下のとおりで、**アセットごとに予算消費率が大きく異なる**。
  ...
  | `hero-idle.webp`（640px 生成） | 最大 320px | 27,860 バイト（約 27.2KB） | **91%** |
  ...
  🔴 **「約 10.6KB」は 256px アセット（`empty-result` 相当）に限った実測値であり、全アセットに当てはまるわけではない**。特に `hero-idle.webp`（640px）は 27.2KB で **個別予算 30KB の約 91% を消費** しており、余裕があるとは言えない。1 ページで同時に読み込むのはロゴ + 状態イラスト 1 枚（例: logo + hero-idle ≒ 30.0KB）で、合計予算 100KB は下回る
