...
- **1 ページ合計 100KB・個別 30KB** を目安予算とする。実測（`gpt-image-2` の透過 PNG を 256px の透過 WebP へ縮小・`sharp` 使用）では 10.6KB 程度に収まり、表示寸法（24〜320px）に対して十分な余裕がある
- 予算内に収まるアセットを、予算超過を理由に別形式（例: 手書き SVG への再作図）へ作り直す必要はない。予算はあくまで **上限のガードレール** であり、それ自体を実装方針の決定根拠にしない（判断根拠は `docs/adr/0015-ai-generated-visual-assets.md`）
