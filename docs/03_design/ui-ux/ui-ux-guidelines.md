...
- **1 ページ合計 100KB・個別 30KB** を目安予算とする（🔴 **本表の KB 表記と消費率はいずれも 1KB = 1024 バイト換算**。個別予算 30KB = 30,720 バイトを 100% とする。行ごとに 1000 / 1024 を混ぜない）。実測値（`ls -l public/images/`・2026-08-22・Issue #359 で `hero-idle` を 16:9 化し `loading` を追加した後の値）は以下のとおりで、**アセットごとに予算消費率が大きく異なる**。
  ...
  🔴 **「約 10.2KB」は 256px の静止画アセット（`empty-result` 相当）の実測値であり、全アセットに当てはまるわけではない**（同じ 256px 生成でも `loading.webp` は約 18.9KB とばらつく）。特に `hero-idle.webp`（16:9 化後も）は 27.0KB で **個別予算 30KB の約 90% を消費** しており、余裕があるとは言えない。1 ページで同時に読み込むのはロゴ + 状態イラスト 1 枚（例: logo + hero-idle ≒ 30.4KB）で、合計予算 100KB は下回る
