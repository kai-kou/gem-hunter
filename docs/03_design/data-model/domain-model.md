...
|---|---|---|---|
| Gem（原石） | `Gem` | **被依存数（実利用）に対して star（注目度）が不釣り合いに小さい OSS**（[ミッション](../../project-mission.md)）。`SP-14` で型として実体化した（`src/domain/model/gem.ts`）。候補プール JSON の 1 エントリに対応し、`packageName` / `repositoryFullName` / `dependentCount` / `stars` / `gemIndex` を持つ。🔵 **`SP-17`（`D-37`）以降、候補プール JSON の 1 エントリは `registry`（下記）も持つ** が、`Gem` 型はまだこのフィールドを写していない（レジストリ別の配信シャード側で使う・`SP-18` / `D-38`）。🔵 **`SP-19` で `registry` を写した派生型 `GemPoolEntry`（下記）を追加した** — `Gem` 自体は据え置きで、レジストリを必要とする Gem 候補の一覧だけが `GemPoolEntry` を使う | 🔴 **MVP では算出しなかったが、`D-27`（`M-5` を「着手する」で通過）により Phase 2 が実装対象へ格上げされ、`SP-14` で実装済み**。「良いリポジトリ」一般を指す語として使わない。生テキスト（`description` 等）は持たない（`D-29`・再配信しない）。🔴 **`Gem.dependentCount` は常に Ecosyste.ms のバッチ取得時点のスナップショット** であり、`RepositoryDetail.starCount` のようなライブ値ではない（表示上は参考値として扱う）。🔵 **`Gem.stars` は使われ方によって出所が分かれる（Issue #310 で刷新）**: `DailyDigest.items`（今日の Gem）としてのインスタンスは `tools/generate_gem_digest.mjs` がバッチ生成時点で GitHub API から取り直した値（取得失敗分は旧値＝ Ecosyste.ms 由来を保持）だが、`GemPoolEntry`（候補プール全体・`/gems` 一覧）としてのインスタンスは再取得の対象外で、`dependentCount` と同じく Ecosyste.ms クロール時点のスナップショットのまま（銘柄ごとにクロール時点が異なり実測で最大 2.7 年ばらつく）。詳細は [ADR 0014](../../adr/0014-zero-query-daily-digest.md) §5 項番 8。UI 表示ラベルは `dependentCount` = 「利用パッケージ数」/ "Used by" に対応する（コード識別子は変更しない） |
...
| shortlist（Gem Index 上位帯） | `GEM_INDEX_SHORTLIST_SIZE` ほか（`src/domain/model/gem-shortlist.ts`・ドメイン層へ移設予定） | 候補プールを `Gem Index` 昇順（同値は `packageName` 昇順でタイブレーク）に並べた上位帯。日次ダイジェストのシャッフル母集団になる | 既定値は 60 件。根拠は [ADR 0014](../../adr/0014-zero-query-daily-digest.md) §2.2.3 が正本（本表では重複記載しない）。🔵 **2026-09-06 に根拠を差し替えた**（Issue #335）: `SP-17` の候補プール入れ替え（12 レジストリ・`D-36` / `D-37`）で旧根拠（star 分布のスナップショット統計）が失効したため、現在は **レジストリ多様性（全 12 中 11 種を含む）とローテーション母数（`limit=5` の 12 倍）** が根拠。再測定は `node tools/analyze_shortlist_distribution.mjs` |
...
- 🔴 **鮮度も異なる**（初見ユーザーのフィードバック⑦・Issue #308）: `RepositorySummary.starCount` / `RepositoryDetail.starCount` は GitHub API のライブ値（`ADR 0005` の TTL でキャッシュ）だが、`Gem.dependentCount` は常に Ecosyste.ms がクロールした時点のスナップショットで、銘柄ごとにクロール時点が異なり実測で最大 2.7 年ばらつく。`Gem.stars` も `GemPoolEntry`（候補プール全体・`/gems` 一覧）としては同様に Ecosyste.ms スナップショットのままだが、`DailyDigest.items`（今日の Gem）としてはバッチ生成時点で GitHub API から取り直した値になった（Issue #310 で刷新）。一覧（`Gem` / `GemPoolEntry`）と詳細（`Repository`）で star 数が食い違って見えるのは、`/gems` 一覧では従来どおり「クロール時点のばらつき」が原因、今日の Gem では「バッチ生成時刻とページ表示時刻の差」が原因（いずれもバグではなく設計上の必然）。詳細は [ADR 0014](../../adr/0014-zero-query-daily-digest.md) §5 項番 8。
