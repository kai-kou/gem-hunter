...
| 同伴 | `GemPoolSearchInput.includeFullNames` / `GemPoolSearchResult.includedCount` | **検索語の照合とは独立に、一覧へ必ず含める `repositoryFullName` 群（および実際に追加された件数）**（`SP-19` 追補・`案3'`・Issue #453・`src/domain/ports/gem-index-port.ts`）。検索結果ページで `GemIndexPort#lookup` によりバッジが付いた候補のうち、一覧側の名前照合（`GemIndexPort#search`）には一致しないものを URL 経由で一覧へ明示的に連れてくる操作を指す | 🔴 **`GemIndexPort#lookup`（バッジ＝候補プールへの所属照会・検索語と無関係）と `#search`（一覧＝リポジトリ名・パッケージ名の単語境界一致・`D-37`）は判定基準が別物である。** バッジは「プールに載っているか」だけで決まり検索語を見ないのに対し、一覧は検索語のトークンに対する AND 一致で決まるため、`q=next.js` のようにバッジが複数件付いても一覧が 1 件に落ちる食い違いが起きうる（Issue #453 の症状）。「同伴」はこの 2 系統の判定基準の差を埋めるための橋渡しであり、`#search` の照合規則そのもの（`D-37`）を変えるものではない。同伴で追加された件数は `includedCount` で返し、UI が「検索語には一致しないが同伴で表示している」ことを注記する。経緯は [`open-questions.md`](../../02_requirements/open-questions.md) `D-36` / `D-37` を参照 |
