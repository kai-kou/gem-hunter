...
読み方: **分 0 ・ 0 時から 2 時間ごと**（00, 02, 04, … 22 時）。
cron は UTC で評価されるため、JST では奇数時ちょうど（01:00 / 03:00 / … / 23:00 JST）に発火する
...
- 先頭の `0` は **分**（毎回 00 分ちょうどに発火する）。ここを変えれば発火する分をずらせる
- `0/N` は「0 時を起点に N 時間ごと」。`0-23/N`（範囲を明示した書き方）でも **完全に同じ**
  （両方を実測し、保存内容・次回発火時刻とも一致することを確認済み）。短い前者を既定とする

### 🔴 `0 */2 * * *`（アスタリスク始まり）を使わない — 分が 0 に固定されないため

Routine の作成 / 更新 API は **`*` 始まりのステップ構文で「毎時」「N 時間ごと」を書くと、分を
...
| **`0 0/2 * * *`** | `0 0/2 * * *` | ✅ **そのまま保存される（分 0 を維持）** |
| `0 0-23/2 * * *` | `0 0-23/2 * * *` | ✅ 同上（上と同じ意味・長い書き方） |

起点を書いたステップ構文（`0/N` / `0-23/N`）なら verbatim 保存される。**`*` 始まりだけがアンカー対象**。
式を差し替えたあとは保存された値を必ず読み返し、分が 0 のままであることを確認する
...
| `cron_expression` | `0 0/2 * * *`（間隔を変えるなら `N` を書き換える。`*` 始まりは分がずれる） |
