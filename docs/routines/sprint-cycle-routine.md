...
**2 時間ごと・毎時 00 分ちょうど** に発火する（`0-23/2` = 0, 2, 4, … 22 時）。
cron は UTC で評価されるため、JST では奇数時ちょうど（01:00 / 03:00 / … / 23:00 JST）になる
（UTC = JST − 9 時間・`datetime-rules.md`）。
...
`0-23/N` の **N を書き換えるだけ**。決定木側の変更は不要。

| 間隔 | cron 式 | JST の発火時刻 |
|---|---|---|
| 1 時間ごと | `0 0-23/1 * * *` | 毎時 00 分 |
| **2 時間ごと（現在）** | `0 0-23/2 * * *` | 01, 03, 05, … 23 時 |
| 3 時間ごと | `0 0-23/3 * * *` | 00, 03, 06, … 21 時 |
| 4 時間ごと | `0 0-23/4 * * *` | 01, 05, 09, 13, 17, 21 時 |
| 6 時間ごと | `0 0-23/6 * * *` | 03, 09, 15, 21 時 |
| 12 時間ごと | `0 0-23/12 * * *` | 09, 21 時 |

JST 側の発火時刻をずらしたいときは開始時を変える（例: `0 1-23/2 * * *` なら JST の偶数時ちょうど）。

### 🔴 `0 */2 * * *`（アスタリスク始まり）を使わない — 分が 0 に固定されないため

Routine の作成 / 更新 API は **「毎時」「N 時間ごと」を `*` 始まりのステップ構文で書くと、分を
作成（更新）時刻の分へサーバー側でアンカーする**（"hourly starting now" 挙動）。実測:

| 送った cron | 保存された cron | 結果 |
|---|---|---|
| `0 * * * *` | `53 * * * *` | ❌ 毎時 53 分にずれる |
| `0 */2 * * *` | `2 */2 * * *` | ❌ 2 時間ごと 02 分にずれる |
| **`0 0-23/2 * * *`** | `0 0-23/2 * * *` | ✅ **そのまま保存される（分 0 を維持）** |

`0-23/N` のように **範囲を明示したステップ構文なら verbatim 保存される**（`*` 始まりだけがアンカー対象）。
上表の式に差し替えたあとは保存された値を必ず読み返し、分が 0 のままであることを確認する
（アンカー挙動に当たっていないかの確認・L-113）。
...
| `cron_expression` | `0 0-23/2 * * *`（上表から選ぶ。`*` 始まりのステップ構文は分がずれる） |
...
3. スケジュールを「2 時間ごと」（`0 0-23/2 * * *`）に設定する
