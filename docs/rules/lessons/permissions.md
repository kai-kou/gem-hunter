...
**残余リスク**: `python3 -c` / `node -e` / `perl -i` / `awk -i inplace` 経由の保護パス書き込みは字面判定で塞げない。`.git/hooks/*` のインストールや `rm .git/index.lock` 等の復旧操作も BLOCK になる（脱出ハッチ `CLAUDE_BASE_DISABLE_WORKSPACE_WRITE_GUARD=1` で通す）。
