...
**再発（2026-09-01・別バリアント）**: 変異テストではなく **ブランチ操作** で同じ事故が起きた。レトロの見送りログへ 10 行追記した直後に作業ブランチを切り替え、`git checkout <別ブランチ> -- content/analytics/retro/deferred_try.jsonl` でファイルを取り直したところ、**未コミットの追記 10 行が別ブランチ版で上書きされて消えた**（再生成できる内容だったため再追記で復旧）。`git checkout <ブランチ> -- <path>` は `git checkout -- <path>`（HEAD から復元）と同じく **未コミット変更を黙って捨てる**。判定基準は同じ — 叩く前に「そのファイルに未コミットの自分の変更が入っていないか」を確認する。
