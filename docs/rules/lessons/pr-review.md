...
テスト・検証 / ドキュメント整合 / Spec 忠実性 / 判定ロジックの適用範囲）が **この欠陥については全て素通りした**
（Layer 1 は同じ PR で別件を 2 件検出している — `select_step2_targets()` の definition drift が CONFIRMED、
SKILL.md の条件重複が PLAUSIBLE。無指摘だったわけではなく、**この結合構造の欠陥だけ** が 7 観点を抜けた）。
...
- 差分 300 行以上で自動起動する Layer 2 は、**Layer 1 の上位互換ではなく別の見落としを拾う層** である
  ことの実例として本エントリを参照する（Layer 1 が 7 観点すべてで **この欠陥を拾えなくても** Layer 2 が
  CONFIRMED を出しうる。本 PR では Layer 1 も別件を 2 件検出しているが、本欠陥は 7 観点のいずれにも
  引っかからなかった）
...
## L-159: 検査を無効化する仕組み（除外マーカー・スキップ条件）を足したら、それ自体を壊す変異テストを置く（2026-09-04・PR #906）

**症状**: 新設した検査ツールに `# selftest-wiring-ok:`（配線検査の除外マーカー）を付けたが、**同じ PR が
実際にその self-test を `run_checks.sh` へ配線していた**。`check_selftest_wiring.py` はマーカーがあれば
`is_wired()` を評価せず即「除外」に分類する順序のため、**配線行を削除しても PASS を返す** 状態になっていた
（Layer 1 のレビュー役が実測して発見・PR #906）。自分で書いた 24 件の self-test も 5 分岐の変異テストも、
この「検査そのものが無効化されている」状態は 1 件も検知しなかった。

**なぜ検知できないか**: self-test と変異テストは **検査ロジックの中の分岐** を対象にする。除外マーカー・
スキップ条件・early return は「検査ロジックへ到達する前に外へ抜ける経路」なので、ロジック内の変異では
一切揺れない。同じ理由で、`--self-test` が緑であることも「その self-test が実行される配線が生きている」
ことを何も保証しない。

**対策**:

- **検査を無効化する仕組みを 1 つ足したら、その仕組みを壊す変異テストを必ず 1 件置く**（マーカーを消す /
  スキップ条件を反転する / early return を外す → 検査が実際に走ることを確認する）
- 除外マーカーを **自分のファイルに付けるときは、その主張が事実か機械で確かめる**。「配線されていない」と
  主張するマーカーは、同じ PR が配線していれば矛盾している（マーカーと実配線の矛盾は機械判定できる）
- 同型: 共通モジュール（`tools/md_fence.py` 等）があるのに独自実装すると、**修正済みのバグを新規ファイルへ
  再導入する**。PR #906 ではフェンス判定を独自トグルで書いた結果、` ``` ` の中の `~~~` で状態が反転し、
  フェンス内の行を本物の証跡として拾う fail-open が再発した（`md_fence.py` では修正済みだった）
...
**原因**: 40 文字のフル SHA ではない値を渡した。実測したのは「短縮 SHA を目視で桁数だけ伸ばした
（実在しない）値」で、実際の head と一致しないため「head が変更された」と判定される。
**桁数を推測で埋めるのは捏造にあたる**（L-113）。`git rev-parse --short HEAD` の生出力（現在の head の
正しいプレフィックス）が受理されるかは未検証なので、いずれにせよ下記の実測値を渡す。
