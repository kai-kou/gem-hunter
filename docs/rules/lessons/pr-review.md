...
## L-165: 重複実装を共通モジュールへ集約した PR に、同じ欠陥クラスの重複が残る（2026-09-07・PR #1064）

**パターン**: 「同じ処理を各ツールが独自実装した結果、片方だけ直って残りにバグが残る」問題を解消するために共通モジュールへ集約する PR を出すと、**その PR 自身が同じ欠陥クラスの重複を残したまま完了しかける**。集約対象として Issue に列挙された箇所だけを直し、**列挙されていない同型の箇所を全数検索しない** ことが原因。
...
2 回目は Layer 1 セルフレビューの「簡素化・再利用」観点が検出できたが、3 回目は **レトロスペクティブの技術レビュアーが指摘するまで気づかなかった**（Layer 1 は「`generate_tokens` の呼び出し」だけを見て、同じモジュールが扱う **トークン種別の集合定義** という別レイヤーの重複を見落とした）。

**対策**:

- 集約 PR では、**Issue 本文が列挙した箇所を直した後に必ず全数検索を追加で行う**（#738 と同じ規律）。検索軸は「呼び出し」だけでなく **「同じ概念を表す定数・集合・正規表現の定義」** も含める（例: `grep -rn "generate_tokens" tools/` で満足せず `grep -rn "FSTRING\|tokenize\." tools/` まで広げる）
- 新設した共通モジュールの docstring に「この 1 関数だけが〜を呼ぶ」といった **正本宣言を書くなら、その宣言が実物と一致することを同じ PR で検証する**（宣言だけ先に書くと、次に読む人が全数検索を省く根拠になってしまう）
- 集約の完了条件に「`grep` の結果が新モジュール 1 ファイルに収まっている」を **実行コマンドごと** 書く（「集約した」という主観的な完了判定にしない）

**保持理由**: 共通化は「同じ欠陥が複数箇所に生まれる」ことを根絶する目的で行うのに、その作業自体が同じ欠陥を残すと目的が達成されないまま Issue がクローズされる。3 回発生しており、レビュー観点（Layer 1 の「簡素化・再利用」）だけでは 3 回目を捕まえられなかった。

---

## L-131: 同一文脈のセルフレビューは自己修正盲点を越えられない。フレッシュ文脈レビューは PR の前に置く（2026-09-11・base#627）
...
**対策（base#627）**: ① `self-reviewer` Step 3.5 で `Skill(code-review)` を `--pre-pr` で実行し CONFIRMED を修正してから
...
対照例 base#586（PR 前に観点別レビュー実施 → PR 後 0 件）。効果は Layer 1 の JSONL 計測（指摘ゼロ PR 率・
