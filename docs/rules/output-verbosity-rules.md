...
| 委譲したサブエージェントの **結論**（完了通知を受け取ってから・§3.1） | **完了待ちの実況**（「完了待機中」「まだ届かない」の繰り返し + 通知確認ツールの連打。待ち方はターンを終えること・`agent-team.md` §F-8） |
...
| `completion-report-rules.md` §1.2 | Stop フック差し戻し後の **続行ターン**（最終報告の **後**）の扱い | 本ファイルの適用対象外（再掲しない / 簡潔に書き直す の切り分けは §1.2 が SSOT）。「アウトカムだけを出す」という同型判断であり矛盾しない |
