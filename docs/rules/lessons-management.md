...
> フックイベント一覧二重管理・境界外リスト A-1〜A-6 の 3 ファイル重複が同根の drift を起こした反省・Issue #144）。
...
| **Lv4 CI**（任意・派生環境向け） | Actions 有効な派生プロジェクトのみ | **本ベースは GitHub Actions を品質ゲートに使わない**（品質チェック用の Actions ワークフローは持たず、クラウド自律実行 + フックで完結する。Gem 候補プールの週次再生成など定期バッチ用のワークフローは別用途として例外）ため、Lv4 CI は既定で設けず Lv3 フックを最終ゲートとする。Actions を使う派生プロジェクトは `lessons-core.md` / `tools/lessons_guard.py` の変更時に `python3 tools/lessons_guard.py check` を走らせる workflow を追加してよい |
