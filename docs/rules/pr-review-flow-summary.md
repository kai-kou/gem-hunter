...
0. **PR 作成前チェック（層 2 の証跡・CI と重複しても省略しない）**: `npm run check`（= `bash tools/run_checks.sh`）を実行し、**結果の Markdown サマリー表を PR 本文に貼る**（貼っていないと `pre-pr-create-check.sh` が PR 作成をブロックする）（`tools/run_checks.sh` 自体の中身は別レーンの持ち物・本ファイルは呼び出し方のみ規定する）。**見出しは `##` 固定で `run_checks` または `npm run check` を含める（`pre-pr-create-check.sh` の検出仕様が SSOT・Issue #405 / PR #456）**: `## run_checks 結果` / `## npm run check 結果` のいずれか（`run_checks` / `npm run check` の部分はバッククォートで囲んでも囲まなくても良い）。同じ見出しが複数回出てきても構わない（いずれか 1 か所が満たしていれば可）。その見出しから次の `##` 見出しまでの間（無関係な別セクションの表・コードフェンス内の例示は不可）に表（`|` 区切り行）があること。🔴 **同じセクション内に `実行時点コミット:` 行を必ず含める**（例: `実行時点コミット: \`abc1234\`` ・#751）。この行が無いと「本文は緑と読めるのに、PR 作成後の追加コミットで実態は赤」という乖離を機械検出できない。**PR 作成後に追加コミットをしたら、`bash tools/run_checks.sh` を現在の HEAD で再実行し、結果表と ` 実行時点コミット:` 行を貼り直す**（検出仕様の SSOT は `pre-pr-create-check.sh`）。🔴 **表の中身も検査される**（#463）: `pre-pr-create-check.sh` 4.5 節は `python3 tools/check_run_checks_evidence.py` を呼び、① 3 列構造（`| チェック | 結果 | 所要秒数 |`）② 結果セルが `PASS` / `FAIL` / `FAIL(timeout)` / `SKIP` のいずれか ③ チェック名が `tools/run_checks.sh` から機械抽出した既知名と一致 ④ **末尾のチェックが含まれている**（＝途中で打ち切っていない）を判定する。**出力を装飾せずそのまま貼ること**（絵文字の付与・行の抜粋・途中までのコピペはブロック対象）。判定内訳は `python3 tools/check_run_checks_evidence.py --body-file <本文> --json` で確認できる。本ツールが検証**しない**こと: 表がその SHA で実際に生成されたか（別ブランチの本物の表をコピーして SHA 行だけ書き換えたケースは通る）と、PR 作成後に HEAD が進んだ場合の再検査（後者はマージ前ゲートの `check_evidence_freshness.py` が担当する）
...
0.8. **完了条件の突き合わせ（対象 Issue がある PR すべて・#776）**: PR 作成前に、対象 Issue 本文の **完了条件セクション配下の箇条書き項目**（`- [ ]` でも素の `- ` でもよい。書式で対象を絞らない）を **1 つずつ引用し、満たしたか / 満たさなかったか**（満たさない場合は理由と切り出し先 Issue 番号）を PR 本文に書く。🔴 **`Closes #N` の有無で適用可否を切り分けない**: `SP-n` のスプリント PR は上記項目 1 の規定により `Closes` を書かないため、`Closes` を条件にすると本リポジトリの主要 PR 経路が丸ごと対象外になる（対象 Issue は `Sprint Goal:` 行・本文の Issue 参照から特定する）。実装を委譲した場合は委譲先の YES/NO 報告（`agent-team-summary.md`「委譲が『空回答・期待外』になる要因」⑤ が求める形式）を根拠にしてよいが、🔴 **親が実物を確認せずに転記しない**。突き合わせを省くと完了条件の一部が未達のまま Issue がクローズされ、後から別 Issue へ切り出す手戻りになる（実例: PR #772 — Issue #478 の明示指定が親の要約で落ちて初版が仕様違反になり Layer 1 が検出、Issue #281 の完了条件 1 件は未達のままクローズされ #774 へ切り出した）
0.9. **ID 採番衝突の確認（`D-n` / `NFR-n` / `SP-n` / `L-n` / ADR 番号を新規採番した PR・#256）**: `python3 tools/check_reserved_ids.py` を実行する。`origin/main` 側で先に確保された番号を独立採番していると **exit 1** で `pre-pr-create-check.sh` 4.9 節が PR 作成を機械ブロックする（`[wip]` コミットのブロック〈項目 0.7〉と同じ位置づけの既知ゲートであり、ハーネスの誤作動ではない）。ID は同じ行を触らず追記されるため git が自動マージし、**マージコンフリクト（`session-concurrency-rules.md` の層 4）には掛からない**。解消手順: 本ツールが種別ごとに出力する「次の空き番号」へ採番し直し、同 PR 内の参照（本文・他ドキュメントからの引用）も併せて更新してから再実行する。exit 2（`git fetch` 失敗・merge-base 未解決）は判定不能でブロックされない（非ブロッキング警告）。🔴 **委譲前に採番表を配る場合は、配る前にも実行する**（事前防止が本命・`agent-team-summary.md`）
...
| **マージ前** | 🔴 **`quality-checks.yml` の check run が緑であることを確認する**（`mcp__github__pull_request_read` の `method="get_check_runs"` または `method="get_status"`。`mcp__github__get_check_run` は check run ID を引数に取るため PR からは引けない）。**赤いままマージしない**（強制力は未配線＝`main` の required status check には未登録なので、運用規律として守る）。⚠️ **例外**: `gem-pool-refresh.yml` が `secrets.GITHUB_TOKEN` で作る `automation/gem-pool-refresh` PR には、GitHub 公式仕様（`GITHUB_TOKEN` 由来のイベントは新しい workflow run を作らない）により **check run が生成されない**。この PR は check run 不在をもって赤とみなさず、同ワークフロー自身の QA ステップが品質を担保する（必要なら `workflow_dispatch` で明示起動して検証できる）。🔴 **あわせて層 2 証跡の鮮度を確認する**（#751）: PR 本文の `実行時点コミット:` 行が現在の head SHA と一致するかを `python3 tools/check_evidence_freshness.py --body-file <本文> --head-sha <head_sha>` で検証する。exit 1（乖離・SHA 行なし）なら **マージせず**、`bash tools/run_checks.sh` を現在の HEAD で再実行して結果表と `実行時点コミット:` 行を貼り直してからやり直す（**PR 作成後に追加コミットをしたら証跡を貼り直す**、が本節の要点）。層 2 は CI の代替ではなく、鮮度確認も CI 緑の確認を省略する理由にはならない。🔴 **check run が「不在」（run 自体が生成されない）ときは、赤でも緑でもない第 3 状態として扱う**（#961）: ① `mergeable_state` を見る（`dirty` はコンフリクトが原因で CI の問題ではない。`origin/main` をマージすれば発火する・L-156）→ ② 上記の既知例外に当たるか → ③ ①②のいずれでもなく不在が続くときに限り、**単発の不発か CI 基盤側の恒久障害（Actions の無効化・課金/クォータ枯渇・`on:` 条件の消失）かを切り分け、恒久障害なら先に `type:bug` を起票したうえで**、層 2 を現 head で実測して FAIL 0 であることを根拠にマージし、その根拠を PR コメントに残す。**不在を理由に PR を放置しない**（判定表は `pr-review-flow.md`「CI check run が不在のときの判定」）。 |
| Layer 0+1 通過後 | **上記の CI が緑であることを確認したうえで** `mcp__github__merge_pull_request`（`merge_method="squash"`）で即マージ |
| **マージ直後** | 🔵 **`site/`（LP）を変更していたら `gh-pages` ブランチへ同期する**（手順の正本は [`site/README.md`](../../site/README.md)・決定は `D-35`）。🔴 **同期を先送りすると、それ以降に作られる `site/` と無関係な全 PR の層 2 証跡（`bash tools/run_checks.sh`）が `check_lp_publish_drift.py` ゲートで FAIL する**（[`site/README.md`](../../site/README.md) 参照）ので即座に行う。続けて 🔴 **本番デプロイの発火条件をゲート判定**（`Sprint Goal:` 行ありなら Step 7 のスプリントレビュー判定へ委譲・無ければ `tools/check_deploy_gate.py` の結果に従う。判断基準・終了コードは `cloudflare-infrastructure.md` §8.2 が SSOT・実行手順は `pr-review-watcher` スキル Step 6/7）→ Slack 完了通知 |
