...
0.7. **自動保全コミットの書き換え（必須・#483）**: ブランチに `[wip]` 件名の自動保全コミットが残っていたら、PR 作成前に自分の作業記憶から **意味のある粒度・意味のあるメッセージ** へ書き換える（件名だけなら `git commit --amend`、粒度が潰れているなら `git reset --soft origin/main` → 論理単位ごとに再コミット → `git push --force-with-lease`）。squash タイトルは単一コミットブランチでその件名を継承するため、放置すると main の履歴が意味を成さなくなる。`pre-pr-create-check.sh` が機械ブロックする
