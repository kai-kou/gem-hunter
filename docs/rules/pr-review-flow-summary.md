...
0.8. **完了条件の突き合わせ（`Closes #N` を持つ PR のみ・#776）**: PR 作成前に、クローズ対象 Issue 本文の完了条件（`- [ ]` 項目）を **1 つずつ引用し、満たしたか / 満たさなかったか**（満たさない場合は理由と切り出し先 Issue 番号）を PR 本文に書く。実装を委譲した場合は委譲先の YES/NO 報告（`agent-team-summary.md`「委譲が『空回答・期待外』になる要因」⑤）を根拠にしてよいが、🔴 **親が実物を確認せずに転記しない**。突き合わせを省くと「機能要件は満たしたが完了条件は未達」のままマージされ、後から別 Issue へ切り出す手戻りになる（実例: PR #772）
