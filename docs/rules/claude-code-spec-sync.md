...
Opus 4.8 の effort デフォルト変更）。なお **新モデル世代のリリース自体は原則として追随作業が不要** （運用ファイルのモデル指定はエイリアス既定・`agent-team.md`「モデル指定の方針」）。追随が要るのは エイリアスの解決先を記した参考表と、effort 既定や API 仕様の破壊的変更のみ。本レーンは定期的にバージョンアップをキャッチアップし、
**内部資産の仕様追随** を自律実行する。
