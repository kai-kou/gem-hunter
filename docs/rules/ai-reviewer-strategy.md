...
**ただし判定器が exit 2（判定不能）を返したときは別扱い**（#881）: spec が不在・壊れているという意味なので、
Layer 2 をスキップ扱いにせず spec を修復してから再実行する（判定器が「起動せよ」と言いながら実行系が
起動できない状態を放置すると、Layer 2 の実施率が 0% のまま緑を返し続ける）。
