...
> **🔴 `/code-review` は自前スキルで置き換え済み（2026-07-21・base#275 → base#280）**: 組み込み `code-review` スキルは
...
- **Layer 1 の実行証跡は PR のレビューとして残っていることが前提**（base#461）。インラインコメントも
  サマリーレビューも無い PR は「Layer 1 未実施」と同じ扱いにし、マージ前に実行する。
- **機械観測層（Issue base#512）**: PR 作成と同一セッションでマージまで進む場合、マージ直前フック
...
- [base#97] Layer 2 定常化 / [base#2485] Gemini 廃止に伴う代替設計 / [base#49] Claude 単独 FAIR への移行決定 / [Copilot 依頼廃止・`/code-review` セルフレビュー必須化タスク] / [base#275] `/code-review` disable-model-invocation 対応 / [base#280] 自前 `code-review` スキル新設（bundled 置換・Layer 1 標準実行手段）
