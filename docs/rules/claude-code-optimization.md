...
**注意**: バックグラウンドエージェントの進捗をポーリングしない。待ち方の正本は `agent-team.md` §F-8「『待つ』の作法」（禁止手段の一覧・例外はそちらが SSOT）。
...
| 変更点 | 内容 | 適用示唆 |
|--------|------|---------|
| **`/code-review --fix`** | 指摘を自動適用（既存 `--comment` に追加） | ⚠️ 本プロジェクトの `/code-review` は自前スキルに置換済みで **これらのフラグは存在しない**（#280）。インライン投稿は常時仕様、修正適用は「`--fix` 相当」としてスキル側に内包（`.claude/skills/code-review/SKILL.md`・#461） |
...
| 2026-08-04 | v2.1.221 | 破壊的変更（影響なし・4件一括） | ① `/ultrareview` の共有履歴なしリポジトリに対するエラーメッセージ改善（`git fetch --unshallow` 誤案内の削除等） → 本プロジェクトは `/ultrareview` を Layer 3 任意レビューとして言及するのみで、共有履歴なしリポジトリを前提にした運用・スクリプトは持たない。② Windows 起動時のプロセス作成時刻取得を PowerShell 起動からネイティブ kernel32 呼び出しに変更 → 本プロジェクトはクラウド/Linux 実行前提で Windows 起動経路に依存する記述なし。③ Claude in Chrome が使用済みタブを自動で閉じるよう変更 → 本プロジェクトは Claude in Chrome 拡張の設定・運用手順を持たない。④ 「Permission mode changed while the auto-mode classifier call was queued」通知の重複表示を承認プロンプトから削除（影響領域ヒント: settings） → `auto-mode classifier`/`Permission mode changed` で横断 Grep した結果 `.claude/settings.json` 含め該当記述なし（`permissions.defaultMode` の auto 系設定も未使用・#289 で確認済みの状態を維持）。4 件とも本プロジェクトの内部資産（rules/skills/hooks/settings）に修正不要 | #408 |
| 2026-08-04 | v2.1.221 | 新機能・検証（5件） | ① VSCode Focus view トグル → 見送り（適用対象外。VSCode 拡張の UI 機能で、本プロジェクトは CLI/クラウド運用のみのため無関係）。② サンドボックス資格情報ファイルの `mode: "mask"`（Linux/WSL 限定・macOS は `deny` フォールバック） → 見送り（時期尚早。具体的な必要性が無く、クラウドではサンドボックス自体が動作しない＝効果はローカル/下流のみ。credential への部分アクセスが必要になったら再検討）。③ `claude plugin validate` のマーケットプレイス/プラグイン名却下警告 → 見送り。本リポジトリは `marketplace.json` を持たず、`.claude-plugin/plugin.json` の名前は `claude-code-base`（英数字+ハイフン）で単純。実機で `claude plugin validate .` を実行し該当警告なしを確認済み（無関係の CLAUDE.md 警告のみ検出）。④ `claude-api` スキル（bundled）への `prompt-audit` サブコマンド追加 → **採用**。`skill-audit` SKILL.md の「陳腐化」観点に、旧世代モデル向け表現の検出手段として参照を追記（低コスト・L-114 型の陳腐化検出を補強）。⑤ `/fork` のセッションが独自 worktree で動作するよう変更 → 見送り（本プロジェクトは `/fork` を運用フローで使用していない） | #409 |
| 2026-08-05 | v2.1.222 | 破壊的変更（影響なし・2件一括） | ① Remote Control の自動起動を repo ローカル設定（`.claude/settings.json`/settings.local.json）ではオンにできなくなり（オフのみ可）、有効化はユーザースコープの `/config` 限定に変更 → 「Remote Control」「remoteControl」「autostart」「auto-start」等で横断 Grep した結果、本プロジェクトの `.claude/settings.json`・settings.local.json（gitignore 済みローカル専用ファイル）・`docs/rules/*.md`・`tools/*.py` に Remote Control 自動起動を設定する記述は皆無（そもそも repo ローカル設定でオンにする運用を持たない）。② ultraplan 機能の削除 → 「ultraplan」で横断 Grep した結果、CLAUDE.md・`docs/rules/*.md`・`.claude/skills/*/SKILL.md`・`tools/*.py` のいずれにも参照なし（本プロジェクトは `/ultrareview`（Layer 3 任意レビュー）のみ言及し ultraplan とは無関係）。2 件とも本プロジェクトの内部資産（rules/skills/hooks/settings）に修正不要 | #422 |
| 2026-08-06 | v2.1.223 | 新機能・検証（3件） | ① `strictKnownMarketplaces`/`blockedMarketplaces` の owner wildcard（`"owner/*"`） → 見送り（managed settings 専用機能で project settings には元々配置不可。本リポジトリはマーケットプレイス制限自体を未使用）。② サブエージェント要求モデルが制限され親モデルで実行される場合の警告 → 見送り（組織の `availableModels` allowlist が無いと発火しない。本プロジェクトはモデル指定をエイリアス既定とし組織側モデル制限は未使用）。③ クラウドセッションでの `/teleport` ヒント → 見送り（対話的ローカル CLI 向け UX 機能で、R-1 等の無人自動化ルーティンには無関係）。3 件とも本プロジェクトの内部資産（rules/skills/hooks/settings）に修正不要 | #432 |
| 2026-08-07 | v2.1.224 | 破壊的変更（1件影響あり・2件影響なし） | ① **200-subagent-per-session spawn cap の撤廃**（"long-running sessions no longer refuse new agents"）→ **影響あり・修正**。`agent-team.md` §F-9 の上限表と `agent-team-summary.md` の要約が「セッション合計 200」を既定値として記載していたため、撤廃を反映（同時実行数 20・ネスト深度 3 は継続。`CLAUDE_CODE_MAX_SUBAGENTS_PER_SESSION` の存続有無は公式 Docs に明記なく未確認と明記）。② managed settings の承認プロンプトが再ログイン/組織切替後に組織設定不変なら再表示されなくなる → 見送り（managed settings は本リポジトリのクラウド運用では使用不可・#2672 で確認済みの前提を維持。関連記述なし）。③ feedback-survey transcript share が同意ベースでモデル設定（システムプロンプト・ツール定義・パラメータ）も送信するよう変更 → 見送り（ユーザー同意が前提のオプトイン機能で、本プロジェクトの rules/skills/hooks に feedback-survey 関連の記述・運用なし） | #439 |
| 2026-08-07 | v2.1.224 | 新機能・検証（7件） | ① `claude self-hosted-runner`（自前マシン/コンテナを Team/Enterprise 向けセッション実行先にする） → 見送り（本プロジェクトは Anthropic 管理クラウド運用のみで、自前インフラへ切り替える必要性なし）。② `archive` プラグインソース（zip over HTTPS からの git/npm 不要インストール） → 見送り（本プロジェクトはプラグイン機構を未使用。`.claude/skills/` 中心の資産構成のまま）。③ ペースト変更取消時の cancel-and-confirm ステップ追加 → 見送り（対話 UI 向けの軽微な UX 改善で、無人自動化ルーティンとは無関係）。④ `ANTHROPIC_BEDROCK_REGION_PREFIX` 環境変数 → 見送り（Bedrock 未使用。Anthropic API を直接利用）。⑤ `crossSessionInbound`/`dialogExpiry` 設定（権限バイパスセッション向けクロスセッションメッセージの承認保留・有効期限） → 見送り（時期尚早。本プロジェクトはクロスセッションメッセージング運用を持たず、CP-4 の並行実行制御は Issue ラベルによる論理ロックで完結している。導入するなら `session-concurrency-rules.md` への影響評価が要るため、実際の運用ニーズが生じたら再検討）。⑥ sandbox 資格情報マスキング拡張（構造化値の `extract`/`onExtractNoMatch`、JWT 対応 `decode:"jwt"`/`maskClaims`、AWS `awsPairs`/`sigv4`。`network.tlsTerminate` 必須） → 見送り（#409 の `mode:"mask"` 見送りと同一理由。クラウド実行はサンドボックス自体が動作せず効果はローカル/下流限定。必要になったら再検討）。⑦ クロスセッション `SendMessage` + `ListAgents`（同一マシン上の他セッションを発見しメッセージ送信・macOS/Linux） → 見送り（時期尚早。ツール自体はセッションに到達済みだが、R-1 は毎回新規セッション・エフェメラル VM で動作するため他ルーティンセッションを実際に発見できるかは未実証。CP-4 の競合防止を Issue ラベルからクロスセッションメッセージングへ置き換えるのはアーキテクチャ影響が大きく、実証と議論型レビューを要するため見送り）。7 件とも見送り・本プロジェクトの内部資産（rules/skills/hooks/settings）に修正なし | #440 |
| 2026-08-08 | v2.1.225 | 破壊的変更（影響なし・2件） | ① `CLAUDE_CODE_OAUTH_TOKEN` を短命トークンへ置換する一時的 401 で headless セッションが壊れる不具合の修正 → 「CLAUDE_CODE_OAUTH_TOKEN」で横断 Grep した結果、本プロジェクトがこのトークンを設定・参照するコード/フック/スキルは皆無（headless (`claude -p`) 呼び出しはセッション内で直接 OAuth トークンを扱わない運用）。② Remote Control セッション再開時、大きな会話が圧縮された後に履歴が壊れる不具合の修正 → 「Remote Control」で横断 Grep した結果、本プロジェクトは Remote Control の自動起動設定・運用手順を持たない（#422 で既に repo ローカル設定でオンにする運用を持たないことを確認済み）。2 件とも Anthropic 側のバグ修正（不具合解消）であり「これまでのやり方がエラーになる」変更ではないため対応不要 | #454 |
| 2026-09-01 | v2.1.257 | 破壊的変更（影響なし・3件） | ① auto mode に Containment Escape rule 追加（クラウドメタデータ資格情報取得・egress 回避・クロステナントアクセスが「想定内」と明示しない限り自動承認されなくなる） → 「Ctrl+E」「metadata-credential」「cross-tenant」「Containment Escape」「egress」（`regress` 等の部分一致を除く厳密一致）で横断 Grep した結果、本プロジェクトの `.claude/settings.json`（`permissions.allow`・`sandbox.network.allowedDomains`）にクラウドメタデータ取得・egress 回避・クロステナントアクセスを許可する記述は元々皆無（`allowedDomains` は github.com/slack.com/api.anthropic.com 等の限定リストのみ）。`permissions.defaultMode` も未設定で auto mode の許可ダイアログ機構自体を利用していない（base#289 で確認済みの前提を維持。本リポジトリでも `permissions.defaultMode` 未設定・`allowedDomains` 限定リストであることを再確認済み）ため、新ルールが従来の自動承認を覆すケース自体が存在しない。② Bash/PowerShell 権限プロンプトの Ctrl+E コマンド説明削除 → UI 表示テキストのみの変更で、本プロジェクトに Ctrl+E への言及・依存記述なし。③ VSCode スクリーンリーダー読み上げ修正 → VSCode 拡張限定の修正で、本プロジェクトは CLI/クラウド運用のみのため無関係。3 件とも本プロジェクトの内部資産（rules/skills/hooks/settings）に修正不要。⚠️ **本行は不完全**（`breaking_keywords` 一致行のみの確認だった）→ 次行（2026-09-03）で見落とし 4 件を訂正 | base#523 |
| 2026-09-03 | v2.1.257 | 訂正（横断確認漏れ 4件・base#548） | base 側 PR #547（`docs/rules/lessons/permissions.md` L-154 に対応）のセルフレビューで、上記 2026-09-01 行の「3件で横断確認済み」が `breaking_keywords` 一致行のみの確認であり CHANGELOG 全変更の確認ではなかったと判明。見落とし: ④ `permissions.ask` が複合コマンド・サブシェル内でスキップされる不具合の修正 → 影響あり（防御強化）。deny/ask の静的評価の抜け穴が塞がれた。`docs/rules/lessons/permissions.md` L-154 に既述。⑤ Bash `Read()`/`Edit()` deny ルールが `< file` リダイレクト・`tac`/`egrep` に適用されていなかった不具合の修正 → 影響あり（防御強化）。本プロジェクトの `permissions.deny`（`Read(.env)` 等）の抜け穴が塞がれた。L-154 に既述。⑥ `defaultMode: "bypassPermissions"` がプロジェクト/ローカル設定（`.claude/settings.json`/`settings.local.json`）で無視されるよう変更 → 影響なし。「defaultMode」で確認した結果、本プロジェクトの `.claude/settings.json` は defaultMode キー自体を持たない（`settings.local.json` は `.gitignore` 対象のローカル専用ファイルで追随対象外）。⑦ auto mode に作業ディレクトリ外初回ファイル読み取り前の一度きりプロンプト追加（`permissions.blockReadsOutsideWorkingDirectories`） → 見送り（L-154 に既述の通り本ベース未採用）。**原因の切り分け**: ④⑤ は「Fixed」始まりの行で `claude-code-spec-sync.md` §2 のデモートポリシー（明示的 breaking 記述がない限り other へ）によりツール起票対象外というポリシー通りの挙動（base PR #547 の作業で個別に発見され L-154 へ既に記録済みだったため実害はなかった）。⑥ は「Changed `defaultMode: …` to be ignored」という言い回しが `breaking_keywords`（`config/claude_code_spec_sync.yaml`）のどの語にも一致しない **真の分類漏れ**（唯一の実バグ）。⑦ は「Added」行で feature 側に正しく分類され得るが、L-154 で既に「本ベースは未採用」と判断済みだったため独立 Issue 化はされていない。**再発防止**: `breaking_keywords` に `"to be ignored"` を追加（CHANGELOG 全文照合で既存語との新規衝突なしを確認済み）。あわせて `claude-code-spec-sync.md` §2 に「『横断確認済み』はツールが breaking 判定した行の確認を指し、CHANGELOG 全行の精読を意味しない」旨を明記し、今後の記録が同じ誤読を招かないようにした | base#548 |
| 2026-09-04 | v2.1.260 | 破壊的変更（影響なし・5件） | ① 1M コンテキストモデル（Opus/Fable）の auto-compact 改善（1M 上限直前で圧縮・巨大コンテキストの復旧圧縮が 10 分でタイムアウトしなくなった） → 見送り（挙動改善のみで既存ルールが前提とする制約なし。「10分」「auto-compact」で `session-compression-rules*.md` を確認したが本変更に依存する記述なし）。② Fable 5.1 で `/effort` をセッション途中で変更してもプロンプトキャッシュが無効化されなくなった → 見送り（純粋な改善で、本プロジェクトは effort 変更がキャッシュを無効化する前提の警告・回避策を持たない。`agent-team.md` の effort 表に影響なし）。③ managed CLAUDE.md（`claudeMd`）がセキュリティ承認ダイアログを発火しなくなった（hooks/shell-command/sandbox/unsafe env は引き続き承認要） → 見送り（managed settings は本リポジトリのクラウド運用では使用不可の前提を維持・base#2672/base#439 で確認済み）。④ サブエージェントが起動するバックグラウンドコマンドの 1 時間制限が撤廃されメインセッションと同様になった → 見送り（「1 時間」「background command」で横断確認した結果、本プロジェクトにこの制限を前提とした記述・回避策なし）。⑤ VSCode の応答再試行時の描画不具合修正 → 見送り（VSCode 拡張限定・CLI/クラウド運用のみのため無関係）。5 件とも本プロジェクトの内部資産（rules/skills/hooks/settings）に修正不要 | base#555 |
| 2026-09-04 | v2.1.260 | 訂正（revert 行の取りこぼし・L-154 訂正） | 上記 2026-09-04 行の「5 件」に **"Reverted the 2.1.259 change applying `Read()` deny rules to Bash arguments"** が含まれていなかった（`breaking_keywords` / `feature_keywords` のいずれにも一致せず「その他」へ落ちて起票前に消えていた）。この revert は v2.1.259 で下流に多発した「auto モードでも `grep -r` / `cd`+`grep` が承認プロンプトになる」事象の解消そのもので、base PR #547 の対策（`--exclude` 自動付与）は v2.1.259 に効かず v2.1.260 では不要と実測で確定 → フック撤去・`docs/rules/lessons/permissions.md` L-154 訂正・`tools/probe_permission_prompts.sh` 新設・`breaking_keywords` に `revert` 追加・本 Step 0 に「新バージョン検知時は分類に関わらずプローブ実行」を追加。議論記録は base 側リポジトリの `content/discussions/auto-mode-prompt-root-cause-20260904/`（本リポジトリには存在しない） | base#558 |
| 2026-09-04 | v2.1.260 | 新機能・検証（9件・全件見送り） | ① フルスクリーンモードの diff パネル（`/diff`） → 見送り（対象外。対話 UI 限定機能で、本プロジェクトはクラウド/無人セッション運用が主体）。② `/cost`・ステータスラインへの prompt-cache miss 原因表示 → 見送り（実利用機会なし。R-1 等の無人ルーティンは `/cost` を呼び出さない運用で、`token-optimization-rules.md` のキャッシュ TTL 記述にも影響なし）。③ `/reload-plugins` の headless 対応 → 見送り（対象外。本プロジェクトはプラグイン機構〈`.claude-plugin/`〉を未使用・base#440 で確認済みの前提を維持）。④ `/advisor` テキスト形式の headless 対応 → 見送り（時期尚早。`/advisor` 自体が experimental 機能で、本プロジェクトは既に `problem-investigation-protocol.md`〈5 ステップ調査〉・`discussion-review`〈議論型レビュー〉・`code-review`/`self-reviewer`〈Layer 1 セルフレビュー〉で複眼検証を構造化済み。stable 化・具体的な適用場面が出たら再検討）。⑤ `oidc.scope_on_refresh`〈Claude apps gateway〉 → 見送り（対象外。Enterprise/Team 管理者向け gateway 設定で、本リポジトリは organization gateway を未運用）。⑥ Claude apps gateway の Desktop policy 新キー（`userPluginMarketplacesEnabled` 等） → 見送り（対象外。同上の理由で organization gateway 未運用）。⑦〜⑨ VSCode 拡張の footer pill effort 表示・session list フィルタ・welcome screen 不具合修正 → 見送り（対象外。VSCode 拡張限定の UI で、本プロジェクトは CLI/クラウド運用のみ）。9 件とも本プロジェクトの内部資産（rules/skills/hooks/settings）に修正不要 | base#556 |
| 2026-09-05 | v2.1.261 | 破壊的変更（影響なし・6件） | ① Vertex AI 起動時、`GOOGLE_APPLICATION_CREDENTIALS` 設定時の Google Cloud プロジェクト検出・`gcloud` 再起動を省略する高速化 → 見送り（「Vertex」「GOOGLE_APPLICATION_CREDENTIALS」で横断確認した結果、`sandbox-rules.md`/`dynamic-workflows-rules.md` の言及は起動時認証情報再検出とは無関係）。② ストリーミング描画時、既描画済みブロックの再チェック省略による高速化 → 見送り（クライアント内部最適化のみ）。③ auto mode でパブリック図表レンダラー URL へのコンテンツ埋め込みアップロードが要求時のみ自動承認に変更 → 見送り（excalidraw/mermaid.live/draw.io/diagrams.net/plantuml で横断確認した結果、該当ツール・スキルなし）。④ プロンプトの単語編集キーが Bash 準拠に変更（Ctrl+W 等）・`keybindingFlavor` 無効化 → 見送り（該当記述なし）。⑤⑥ VSCode の MCP サーバーダイアログ改善・セッション一覧ホロー表示 → 見送り（VSCode 拡張限定・CLI/クラウド運用のみのため無関係）。6 件とも本プロジェクトの内部資産（rules/skills/hooks/settings）に修正不要。権限プローブ（`probe_permission_prompts.sh`）も OK（挙動退行なし） | base#572 |
| 2026-09-05 | v2.1.261 | 新機能・検証（12件・全件見送り） | ① `/status`・`claude doctor` への Organization policy 未読込理由の表示追加 → 見送り（managed settings/organization gateway 未運用の前提を維持・base#439/base#2672 で確認済み）。② `bashOutputMaxChars`/`taskOutputMaxChars` 設定（インライン出力上限を最大 128K 文字まで拡張） → 見送り（`token-optimization-rules.md` に既存の出力上限で困っている記述はなく、具体的な必要性が生じていない。将来大出力のファイル保存切替が問題化したら再検討）。③ `--append-subagent-system-prompt-file`（サブエージェントシステムプロンプトをファイルから読み込み） → 見送り（`claude -p` 呼び出し箇所〈`tools/run_deep_research*.py`/`run_discussion_review.py`/`native_fallback.py`/`discussion_review_trigger.py`/`skill-creator` 配下〉を横断確認したが `--append-system-prompt` 系フラグの使用・長大サブエージェントプロンプトのコマンドライン受け渡し運用はなし）。④ `/skill-doctor`（未使用スキルとそのコンテキストコスト表示） → 見送り（本プロジェクトは同等の陳腐化・肥大化検出を `skill-audit` スキルで既に運用中。ビルトインコマンドとしての追加であり rules/skills 側の変更は不要）。⑤ managed settings gateway 403 エラーメッセージの改善 → 見送り（同上・managed settings 未運用）。⑥〜⑫ VSCode 拡張の walkthrough・fold button・archive session・group/badge/usage meter 不具合修正（7件） → 見送り（対象外。VSCode 拡張限定の UI 機能で、本プロジェクトは CLI/クラウド運用のみ） | base#573 |
| 2026-09-12 | v2.1.269 | 破壊的変更（影響なし・7件） | ①〜③ 日本語/中国語/韓国語プロンプト候補フィルタ改善・長時間セッションでの transcript 更新高速化・Artifact データベース読み取り〈scratchpad 保存時〉の working-folder 承認省略 → 見送り（クライアント内部最適化、または本プロジェクトが未使用の Artifact db 読み取り機能）。④〜⑥ VSCode 拡張のセッションリネーム保持・右クリックメニュー整理 → 見送り（VSCode 拡張限定・CLI/クラウド運用のみのため無関係）。⑦ Claude Tag の失敗通知が Slack レート制限で欠落する不具合の修正（リトライ化） → 見送り（本プロジェクトは Claude Tag／Slack スレッド運用を持たず `slack_notify.py` は Webhook 経由の一方向通知のみ）。権限プローブ（`probe_permission_prompts.sh`）は OK（挙動退行なし）。⚠️ 要精読 38 件（`others_hinted`）も横断確認: `!` 否定権限ルール・`Bash(tee:*)` allow ルール・`CLAUDE_CODE_BG_TASKS_REPORT_RUNNING`・attribution reminder 関連の記述は `.claude/settings.json`/`docs/rules/*.md`/hooks のいずれにも該当なし（`tee` はテスト用パイプ検知パターンとしてのみ出現）。7 件+38 件とも本プロジェクトの内部資産（rules/skills/hooks/settings）に修正不要 | base#639 |
| 2026-09-16 | v2.1.273 | 破壊的変更（影響なし・4件） | ① `2.1.268` の revert（Read/Edit deny を eval/env -C 等の解析不能な Bash 行にも適用していた変更を撤回。`time -p make build` 等の誤検知が prompt に戻る） → 見送り。`.claude/settings.json` の `permissions.deny` は `Read()`/`Write()`/`Edit()` のパスパターンのみで `Bash()` パターンを持たず、`pre-tool-use-router.sh` の第2層防御（機密ファイル Bash アクセスブロック）も eval 経由の難読化をコメントで明示的にスコープ外としコンテナ隔離を残余リスクの受け皿にしている＝ネイティブ classifier の deny/ask 挙動変更から独立。② 長時間セッションでの hook/サブエージェント活動再処理の高速化 → 見送り（パフォーマンス改善のみ）。③ [web] routines のコネクタ再追加後の旧接続問題修正 → 見送り（org connector 未使用）。④ [web] 新規ユーザー向けデスクトップアプリ誘導画面の削除 → 見送り（UI のみ）。権限プローブ（`probe_permission_prompts.sh`）は OK（挙動退行なし）。⚠️ 要精読 27 件（`others_hinted`）も横断確認: 「サブエージェント/バックグラウンドエージェントの token usage/model id 欠落時 false-failure 誤報告」修正は `agent-delegation.md` L-121 のワークアラウンド対象外（別事象・恩恵のみ）、「context meter/auto-compact の advisor-tool ターン二重カウント」修正は `session-compression-rules*.md` に閾値依存の記述なし、残り 25 件（MDM/`blockReadsOutsideWorkingDirectories`/UI/SSO/OTEL 等）は本プロジェクト未使用の機能。4 件+27 件とも本プロジェクトの内部資産（rules/skills/hooks/settings）に修正不要 | base#675 |
