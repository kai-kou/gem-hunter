...
> 🔴 **事実調査は Claude の仕事、決定だけがユーザーのもの。** 環境から確認できる事実
> （ファイル・設定・API の応答・既存ルールの有無・過去の Issue）は **すべて自分で調べ切ってから**、
> 真に人にしか判断できない論点だけを残す。ユーザーに聞いてよいのは「どちらを選ぶか」であって
> 「どうなっているか」ではない。**調べれば分かることを人に聞いた時点で L-077 に該当する。**
> 調査に時間がかかる項目があっても、そこで止まらない — 走らせたまま、その結果に依存しない
> 論点を先に片付ける（依存する論点だけが結果を待つ）。
...
> ⚠️ **ロジックバグ・想定外挙動型（カテゴリ G）だけは、この Step の前に再現ループを作る**。
> 検証手段が無いまま並列調査を投げると、検証不能な仮説が並ぶだけになる。詳細は
> 「典型障害カテゴリ別チェックリスト」の G を参照。
...
### G. 「ロジックバグ・想定外挙動」型（A〜F と違い、先に再現ループを作る）

A〜F は **環境側の障害**（設定・到達性・ファイル・依存・実行基盤・データ形式）で、原因の候補が有限個に
絞られているため Step 3 の並列調査が直接効く。本カテゴリはそうではない。`tools/` の Python・
`.claude/hooks/` の Shell が **文法的には正しく動いているのに結果が間違っている** という型で、
コードを読んで立てた仮説はいくらでも作れてしまい、**どれが正しいかを判定する手段がない** のが本質。

したがって本カテゴリだけは順序制約が入る。

> 🔴 **Step 2 で既知の対策にヒットしなかったら、Step 3 の専門チーム展開の前に一度立ち止まり、
> 再現ループを先に作る。** ループが `red`（バグを捉えて失敗する状態）になるまで仮説に進まない。

**なぜ順序を変えるか**: `red` になるループが 1 本あれば、そこから先の二分探索・仮説検証・計装は
すべてそれを消費するだけの機械作業になる。ループがないまま並列調査を投げると、各エージェントが
検証不能な仮説を持ち寄り、メインがどれを信じるか選べない（もっともらしさで選ぶと誤診する）。

#### 完了条件: `red` になる tight なループが 1 本ある

**1 つのコマンド**（スクリプトのパス・テスト起動・curl）を名指しでき、**それを実際に 1 回以上実行して
出力を示せる**こと。そのうえで次の 4 条件をすべて満たす:

```
[ ] red-capable: 実際のバグ経路を通り、ユーザーが述べた症状そのものを assert する
                （「エラーなく動く」では不合格。このバグを捉えられ、直れば緑になること）
[ ] deterministic: 何度動かしても同じ判定（非決定バグは、まず再現率を上げてから）
[ ] fast: 分ではなく秒（非決定バグで試行を重ねる場合は、1 試行あたりが秒であればよい）
[ ] agent-runnable: 無人で実行できる（人手が要るなら対話スクリプトに閉じ込める）
```

コードを読んで仮説を立てている自分に気づいたら、**このコマンドが存在するかを先に確認する**。
存在しないなら手を止めてループ作りに戻る。

#### ループの作り方（上から順に試す）

```
1. 失敗するテスト（そのバグに届く粒度で。単体・結合・E2E は問わない）
2. curl / HTTP スクリプト（起動中のローカルサーバに対して）
3. CLI 実行 + フィクスチャ入力（既知の正しい出力との差分）
4. ヘッドレスブラウザスクリプト（DOM / コンソール / ネットワークを assert）
5. 捕捉したトレースの再生（実リクエスト・ペイロード・イベントログを保存して単体で流す）
6. 使い捨てハーネス（依存をモックして該当経路だけを 1 関数呼び出しで叩く）
7. property / fuzz ループ（「たまに出力が壊れる」型。乱択入力を大量に流す）
8. bisect ハーネス（既知の良い状態と悪い状態の間で起きたなら git bisect run にかける）
9. 差分ループ（旧版と新版・2 つの設定に同じ入力を流して出力を diff）
```

**ループはプロダクトとして扱う**。一度できたら締める（速くする・assert を症状そのものに寄せる・
時刻や乱数や外部依存を固定する）。30 秒かかって不安定なループは、無いのとほとんど変わらない。

**非決定バグ**: きれいな再現を目指すのではなく **再現率を上げる**。上の `fast` は
**1 試行あたり** にかかる時間を指すので、試行を重ねること自体は条件と矛盾しない
（1 試行が秒なら、束ねて回しても手が止まらない。1 試行が分なら、まず 1 試行を締めてから重ねる）。
100 回ループ・並列化・負荷付与・
タイミング窓の縮小・sleep 注入。50% で出るバグはデバッグできる。1% はできない。

**どうしてもループを作れないとき**: 手を止めて明示的にそう言う。試した手段を列挙し、
`user-confirmation-minimization.md` の判定に載せる。**ループなしで仮説に進まない。**

#### 計装は撤去可能にする

デバッグ用のログには `[DEBUG-a4f2]` のような一意プレフィックスを必ず付ける。撤去が grep 一発になる
（タグの無いログは残り続ける）。プローブは Step 3/4 で立てた仮説 1 つに 1 つ対応させ、**一度に 1 変数だけ**
動かす。性能劣化の調査ではログではなく計測（タイマー・プロファイラ・実行計画）を先に取る。

#### 完了チェック（Step 5 に入る前）

```
[ ] 元の再現手順で再現しなくなった（本カテゴリで作ったループを再実行して確認。
    絞り込んだ最小ケースだけでなく、Step 1 で記録した元の症状でも確認する）
[ ] 回帰テストが通る。ただし **正しい seam が無いなら、それ自体が finding**
    （テストが本物のバグ経路を通らない場所に書かれると誤った安心を生む。書けない事実を記録する）
[ ] `[DEBUG-` の残存ゼロ（grep で確認）
[ ] 使い捨てハーネス・プロトタイプを削除した
[ ] 当たっていた仮説をコミット / PR 本文に 1 行残した（次に踏む人が学べるように）
```
...
Step 2: 既存ナレッジ検索
  ...
  └─ ヒットなし
       ...
       Step 4: 統合・根本原因特定
         ...
         │   → core-principles.md の既約境界外リスト該当
         │   → エスカレーション報告テンプレートで報告
         │
...
### ユーザー判断が必要な理由
- [ ] core-principles.md の既約境界外リスト該当（main 直接 push / YouTube public 化等）
...
- `docs/rules/autonomous-operation-policy.md` — Bounded Autonomy・既約境界外リスト
