...
| 8. ID 採番衝突検査（#256） | `origin/main` と独立採番された `D-n` / `NFR-n` / `SP-n` / `L-n` / ADR 番号を検出（層 4 の自動マージをすり抜ける経路を塞ぐ） | 採番表の配布前（親の手動実行）と PR 作成直前（`pre-pr-create-check.sh` 4.9 節） |
...
- `status:blocked` を解除するだけで `status:in-progress` を付けない（着手しないなら `status:waiting-claude` にしてコメントで明記する・#306）
