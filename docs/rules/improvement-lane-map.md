...
>
> **retro-try 自身の出口（レーンをまたがない別経路・base#563）**: 上記のリファインメント 4 出口とは別に、
> 振り返りレーン内の `retro-try-handler` Step 1.5（TTL 自動クローズ・`not_planned`）が
> 「実装する」以外の退出経路を持つ（数値の SSOT は `retrospective-rules.md`「WIP 制御」）。
> この出口は改善 Issue レーンのリファインメントを代替しない（対象 Issue の集合はルール 5 のとおり）。
