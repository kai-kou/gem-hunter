...
```bash
gh variable set SLACK_BOT_TOKEN -R kai-kou/gem-hunter --body "xoxb-..."
gh variable set SLACK_CHANNEL_ID -R kai-kou/gem-hunter --body "C0XXXXXXXXX"
# ... 残りの変数
```
...
```bash
gh variable set XXX -R kai-kou/gem-hunter --body "値"
```
...
# gh CLI で直接操作
gh variable get SLACK_BOT_TOKEN -R kai-kou/gem-hunter
gh variable set SLACK_BOT_TOKEN -R kai-kou/gem-hunter --body "xoxb-..."
gh variable delete SLACK_BOT_TOKEN -R kai-kou/gem-hunter
```
...
| 変数名 | 用途 | 必須 | 値の例 |
|--------|------|------|--------|
| `ENABLE_PROMPT_CACHING_1H` | プロンプトキャッシュの TTL を 1 時間に維持する。既定はサブスクなら 1 時間だが、**Usage Credits 利用中・API キー / クラウドプロバイダ経由では 5 分に落ちる**（[公式](https://code.claude.com/docs/en/costs)）ため、本 env でその条件下でも 1 時間を保つ。毎時パイプラインで CLAUDE.md・常駐ルール群を繰り返しロードするため、キャッシュ読込の90%割引でコスト削減（5分超のターン間隔が常態のため確実に元が取れる）。Claude Code BP差分 P-9（#2672・専門チームレビュー GO） | 推奨 | `1` |

### ハーネス（hooks）トグル変数

フック個別の挙動を切り替える変数。**既定値と詳細は各スクリプト冒頭のコメントが SSOT** で、本表は所在の索引として持つ。
...
| `E2E_CHROMIUM_EXECUTABLE` | Playwright の Chromium 実行ファイルパスを明示上書きする。既定解決が失敗する環境（#629）の回避策。実在チェックはせず指定値をそのまま使う。未設定時は既定解決 → `PLAYWRIGHT_BROWSERS_PATH` 配下の自動探索にフォールバック | 任意 | `/opt/pw-browsers/chromium-1194/chrome-linux/chrome` |
...
GH_TOKEN で gh CLI 認証（Claude.ai 環境変数から取得）
  ↓
gh variable list -R kai-kou/gem-hunter --json name,value
  ↓
各変数について:
