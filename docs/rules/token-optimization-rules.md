...
| 2026-09-06 | 151,876 B | +5,170 B | #767 / #776 / #1019（PR #1024）: 委譲プロンプト必須項目に 3 系統を追加した。① `agent-team-summary.md` に規約準拠タスク向けの必須 3 点を新節として追加（+約 2.0KB）② 同 `#367` 節へ ⑤（Issue 本文の対応方針・完了条件の原文転記と委譲先への YES/NO 報告要求）を追加（+約 0.6KB）③ 同 `#474` 節へ項目 0（着手前の既存共通モジュール提示・検索手段・機械検査対象外の理由）を追加（+約 1.8KB）④ `pr-review-flow-summary.md` に項目 0.8（親の完了条件突き合わせ・+0.75KB）。いずれも削減対象外②（実観測ベースの行動規範）に該当する: ① は PR #765 で規約の節見出しを誤読して対象の半分にしか適用しなかった実測、② は PR #772 で親の要約が仕様を落とし完了条件 2 つが未達のまま PR に載った実測、③ は PR #1018 で `tools/ts_source.py` の存在を伝えず Issue #612 で解決済みの欠陥クラスを再導入した実測が根拠で、**いずれも委譲プロンプトへ書かない限り発火しない**（サブエージェントは `docs/rules/` を自動では読まない）。判定ロジックを持てる部分は Hot 層外に置いている（並行安全プリアンブルの逐語一致は `tools/check_delegation_preamble.py`、共通モジュール列挙の妥当性は領域知識依存のため機械検査せず Layer 1 セルフレビューが見る、と③本文に明記した）。🔴 **本行で再棚卸しの合図 ② が新たに成立した**（実測 151,876 B が基準 137,697 B の +10.3% で閾値 +10% に到達。前行時点では +6.5% だった）。合図 ①（増減ログ 12 行）も継続。棚卸しは #798 が追跡中で本 PR のスコープ外のため、#798 へ閾値到達を記録して引き継ぐ |
