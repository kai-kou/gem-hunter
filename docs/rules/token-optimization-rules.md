...
| 2026-09-03 | 137,156 B | **本リポジトリの基準** | ベース適用コミット `399adad` 時点の実測（14 ファイル）。ベース側の記録値（85,320 B）は参考にとどめ、差分計算の起点にしない（`apply-base` SKILL.md §6）。前基準（同日・130,396 B）からの +6,760 B は本適用で取り込んだ base#543 由来の `completion-report-rules.md` §1.2（Stop フック差し戻し後の続行ターンの規律）と、`lessons-core.md` の Warm 層索引 2 行（L-153 / L-154）が内訳で、いずれも削減対象外②（実観測ベースの行動規範）に該当する。**本行を新しい基準とする** |

**表の最新行が本リポジトリの基準** である。以後 Hot 層を触る PR は `python3 tools/check_hot_budget.py` を実行し、本表に 1 行足す（実測とログ最新行が一致していることが合格条件）。🔴 **実測 137,156 B は基準を打ち直した値であって「妥当な水準」ではない**。ベース側の到達値（85,320 B）に対し +51.8KB あり、次の棚卸しでは「削減対象外 3 区分（A-1〜A-6 / 実観測ベースの行動規範 / Haiku テンプレート）に該当しない箇所」から no-op テスト（上記）と single source of truth の 2 軸で削減余地を洗う。
