...
| 日付 | 実測 | 差分 | 追加の正当化 / 相殺 |
|---|---:|---:|---|
| 2026-08-04 | 68,713 B | 基準 | #369 の再棚卸し後の到達値（#325/#328/#367/#375 分の行動規範は削減対象外と再確認のうえ残置し、代わりに手順・具体例 3 件を Warm へ降格） |
| 2026-08-17 | 79,491 B | **+10,778 B** | ⚠️ **未記録だった既存ドリフト**（13 ファイル構成のまま基準から乖離していた分。#13 の棚卸しで実測して初めて判明。本 PR 起因ではない） |
| 2026-08-17 | 89,123 B | +9,632 B | #13: `sprint-development-rules.md` を Hot 追加（8,505 B。全文 17,374 B のうち約半分を `-detail.md` へ Warm 分離した後の値）+ `user-confirmation-minimization.md` に確認 2 系統の定義を追記（+1,127 B）。**いずれも代替の強制レイヤが無い行動規範**（削減対象外②）のため Hot 必須 |

**現行の実測は ~89.1KB / ~22,300 トークン（14 ファイル）**。直近の基準 ~68.7KB に対して **+30%** で、うち **+10.8KB は未記録だった既存ドリフト**。

🔴 **再棚卸しの合図が立っている**（増減ログが 3 行に到達 + 基準から +30%）。次の棚卸しでは、本追加分（#13 の行動規範）ではなく **未記録ドリフト 10.8KB の内訳** を先に洗う（どのファイルがいつ膨らんだかが追跡できていないため）。内訳の洗い出しは **#15** で追跡する。
