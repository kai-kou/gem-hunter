...
| 2026-09-02 | 122,931 B | +2,368 B | #725 / #768: `agent-team-summary.md` に 2 項目追記。① 「独立した複数対策を 1 委譲に含めるときはデータフロー干渉を検証させる」を新設（#725・実例: `CHANGED_FILES:` 明示ブロック対策と入力切り詰め対策を 1 体に同時委譲したところ、切り詰めが明示ブロックを消してしまい片方が他方を無効化した。単体 self-test は両方緑だった）② #93 の禁止対象に「作業ツリーを移動するコマンド」（`git checkout <branch>` / `git switch` / `gh pr checkout`）を明示（#768・実例: レビューのファインダーがブランチを移動し親の作業ツリーが `main` のまま残った）。**いずれも代替の強制レイヤを持てない行動規範**（削減対象外②）— 実際に観測された干渉・誤認事故を防ぐための記述で、機械検査には落とし込めない |
