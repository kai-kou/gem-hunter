...
| 2026-09-01 | 120,064 B | +371 B | #281 / #478 / PR #772: `lessons-core.md` の L-103 に `stop-pr-check.sh` の TTL 抑制条件を明記（+約 130 B）・`CLAUDE.md`「Markdown 出力ルール」に `check_markdown_table_columns.py` の実行を追記（+約 56 B）。**ドキュメントが実装より強い保証を主張する状態を解消するための記述**（削減対象外②）— L-103 は「push 済み未 PR を機械検知する」と無条件の保証を書いており、抑制条件を書かないと「リマインドが出ない＝ PR がある」と誤読される。残る +185 B は本 PR 以前からログ未記録だった累積分（Hot 層ファイルの差分ゼロを実測して確認済み） |
