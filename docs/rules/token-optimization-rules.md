...
| 2026-09-02 | 125,328 B | +4,765 B | #725 / #768 / #720 / #719 / #686: Hot 層 4 ファイルへ追記。① `agent-team-summary.md` に「独立した複数対策を 1 委譲に含めるときはデータフロー干渉を検証させる」を新設（#725・実例: `CHANGED_FILES:` 明示ブロック対策と入力切り詰め対策を 1 体に同時委譲したところ、切り詰めが明示ブロックを消して片方が他方を無効化した。単体 self-test は両方緑だった）② 同ファイル #93 の禁止対象に「作業ツリーを移動するコマンド」（`git checkout <branch>` / `git switch` / `gh pr checkout`）を明示（#768・実例: レビューのファインダーがブランチを移動し親の作業ツリーが `main` のまま残った）③ `core-principles.md` CP-1 のスコープ境界に「既存の齟齬でも自分の追加分が影響を受ける位置なら同じ PR で直す」を追加（#720）④ `sprint-development-rules.md` `SD-1` の入力境界表を解釈ロジック変更にも適用（#719）・`SD-2` の完了条件に変異対象の選定基準（本番の主コードパス必須）を追加（#686）。**いずれも代替の強制レイヤを持てない行動規範**（削減対象外②）— 実際に観測された干渉・誤認・見逃し事故を防ぐための記述で、機械検査には落とし込めない（③④の機械化は #720 / #686 で明示的に見送りを判断済み） |
