...
### 削減効果・予算の推移（#146 → #324 → #369 → base#469 で再校正）

| 指標 | 当初（8ファイル構成時） | #146 棚卸し前（2026-07-10） | #146 棚卸し後 | #324 棚卸し後（2026-07-26） | #369 棚卸し後（2026-08-04） | **base#469 棚卸し後（2026-08-23）** |
|------|------|------|------|------|------|------|
...
**削減対象外（意図的に残す）**: ① A-1〜A-6 の既約境界外リスト ② 実観測ベースの行動規範 lessons（記事の削除基準 "specific, demonstrable failure mode" に照らすと残す側）③ Haiku サブエージェント向けの明示的な出力ルール（Claude 5 世代ではないため「判断に委ねる」の適用外）。**base#469 の再点検でもこの 3 区分は維持**（該当箇所は全件この基準で残置と再確認）。ベース側が今回削減できたのは削減対象外に該当しない箇所（CP-1 の長大な境界線説明・PR レビューサマリーのセッション復帰プロセス説明）のみで、以後の追加は下記の機械チェックに従う。
...
#### 過去の実測記録（2026-08-04 〜 2026-08-23・アーカイブ）
