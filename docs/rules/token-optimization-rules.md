...
| 2026-09-05 | 142,252 B | +1,542 B（本タスク分は +683 B） | #961: `pr-review-flow-summary.md`「マージ前」に **check run 不在（緑・赤に次ぐ第 3 状態）の判定順** を追記した（+683 B）。削減対象外②（実観測ベースの行動規範）に該当する: PR #960 で `mergeable_state: dirty` により `quality-checks.yml` の run が生成されず、条文が緑・赤しか規定していなかったため 「放置」と「規定なしの裁量マージ」の二択に落ちた（L-156 の 2 回目の再発で、教訓ではなく導線が欠けていた）。判定表の本体は Warm 層（`pr-review-flow.md`）へ置き、Hot には切り分けの順序と「不在を理由に PR を放置しない」規律だけを残した。**残る +859 B は本タスク以外の並行変更分で、その内訳は本行では追跡していない** |
