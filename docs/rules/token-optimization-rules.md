...
| 2026-09-06 | 145,187 B | +1,470 B | #775 / PR #1006: `agent-team-summary.md` の検査ツール委譲の必須項目（#474）に 「4. 変異テストの実測」を追加し、既存項目を 5〜7 へ繰り下げた（+1,470 B）。**代替の強制レイヤを持てない行動規範**（削減対象外②）— 実観測: PR #772 の fan-out でサブエージェントが変異テストを「⚠️ PASS（カバー不十分）」と報告したまま完了扱いにし、後の調査で **変異文字列が源文に存在せず変異が一度も適用されていなかった**（＝「実施した」という報告自体が何も検証していなかった）と判明した。`sprint-development-rules.md` `SD-2` は親セッション向けの規律であり、サブエージェントは `docs/rules/` を自動では読まない（#367）ため、委譲プロンプトに書かない限りこの規律は届かない。**手順の詳細は複製せず `SD-2`「変異テストの進め方」と `tools/mutation_guard.sh`・L-131 / L-133 へ一方向参照している**（Hot 層に持ち込んだのは委譲プロンプトへ書くべき指示のみ）。⚠️ 再棚卸しの合図 ①（ログ 9 行）・②（基準 137,697 B 比 +5.4%）は継続して立っており、棚卸しは #798 / #773 が担当する |
