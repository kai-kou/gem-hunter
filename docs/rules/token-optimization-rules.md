...
**削減対象外（意図的に残す）**: ① A-1〜A-6 の既約境界外リスト ② 実観測ベースの行動規範 lessons（記事の削除基準 "specific, demonstrable failure mode" に照らすと残す側）③ Haiku サブエージェント向けの明示的な出力ルール（Claude 5 世代ではないため「判断に委ねる」の適用外）。**#469 の再点検でもこの 3 区分は維持**（該当箇所は全件この基準で残置と再確認）。ベース側が今回削減できたのは削減対象外に該当しない箇所（CP-1 の長大な境界線説明・PR レビューサマリーのセッション復帰プロセス説明）のみで、以後の追加は下記の機械チェックに従う。
...
| 日付 | 実測 | 差分 | 追加の正当化 / 相殺 |
|---|---:|---:|---|
| 2026-08-30 | 115,390 B | 基準 | **ベース適用コミット `c07efc1` 時点の実測（14 ファイル）**。前回の基準行（2026-08-19・91,960 B）以降、増減ログに記録されたのは 2 行だけで、最終記録（2026-08-23・101,623 B）から本実測まで **+13,767 B が未記録のまま累積** していた（`tools/check_hot_budget.py` の「実測とログ最新行の乖離」判定が本適用で初めて機械化された）。🔴 ベース側の記録値（77,828 B）は参考にとどめ、差分計算の起点にしない（`apply-base` SKILL.md §6）。**本行を新しい基準とし、これ以前の記録は下の「過去の実測記録」へアーカイブした** |

**この行が本リポジトリの基準** である。以後 Hot 層を触る PR は `python3 tools/check_hot_budget.py` を実行し、本表に 1 行足す（実測とログ最新行が一致していることが合格条件）。🔴 **実測 115,390 B は基準を打ち直した値であって「妥当な水準」ではない**。ベース側の到達値（77,828 B）に対し +37.6KB あり、次の棚卸しでは「削減対象外 3 区分（A-1〜A-6 / 実観測ベースの行動規範 / Haiku テンプレート）に該当しない箇所」から no-op テスト（上記）と single source of truth の 2 軸で削減余地を洗う。
