...
**この行が本リポジトリの基準** である。以後 Hot 層を触る PR は `python3 tools/check_hot_budget.py` を実行し、本表に 1 行足す（実測とログ最新行が一致していることが合格条件）。🔴 **実測 130,396 B は基準を打ち直した値であって「妥当な水準」ではない**。ベース側の到達値（78,724 B）に対し +51.7KB あり、次の棚卸しでは「削減対象外 3 区分（A-1〜A-6 / 実観測ベースの行動規範 / Haiku テンプレート）に該当しない箇所」から no-op テスト（上記）と single source of truth の 2 軸で削減余地を洗う。
