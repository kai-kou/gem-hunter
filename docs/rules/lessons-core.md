...
- 🔴 **未追跡ファイルは Stop フックの WIP 自動コミットに含まれない**（`git add -u`・#94 の方針）。クリーンアップ直前に `.git/untracked-backup/<時刻>/` へ自動退避される（#193・直近 5 世代）ので、**消えたと思ったらまずそこを見る**。ただし退避は最後の保険なので、Stop 時に警告が出たらその場で add・削除・`.gitignore` のいずれかに決める
...
例外を満たす。判断任せにせず `stop-pr-check.sh` が push 済み未 PR ブランチを終了時に機械検知する（同一セッション・同一ブランチで PR の実在を確認済みの間だけ再掲を抑制する。TTL 既定 30 分・`PR_CHECK_CONFIRMATION_TTL_MINUTES` で上書き可）。
...
後者なら本文に出さず思考に留め、**統合アウトカムを 1 回** で報告する。**2 回目以降の** ツール呼び出しの
直前に「これから〜する」と逐次宣言しない（無言で実行）。例外は ① **着手時のプラン提示**
（`output-verbosity-rules.md` §1.1・下記 L-124 の必須義務）、② `progress-reporting-rules.md`（制作系 5 分超）、
③ `session-safety-rules.md` ルール 4（大きな Read 直後の短い事実 1 行）。詳細は
...
⚠️ **output style と重複しているが降格しない**: output style はプロジェクト側で差し替え可能なレイヤであり、差し替えた瞬間に規律が静かに失われる（衝突解決規則は `token-optimization-rules.md`「2 軸が衝突したときの解決規則」・#19）。

---

## L-124: 着手時のプラン提示を省略し、無言で長時間作業に入ってしまう

**パターン**: ユーザー指示を受けた直後、何をするつもりかを一切伝えないまま作業に入り、ユーザーが
「これから何が起きるのか」を把握できないまま待たされる。L-111（逐次宣言の禁止）を **過剰適用** し、
「タスク着手時 1 回のプラン提示」まで巻き込んで消してしまうのが原因。**Opus / Fable で顕著**:
`orchestrator-directive.sh` が「即座にサブエージェントへ委譲せよ」を注入し、`output-verbosity-rules.md`
§3.1 が「委譲前の宣言」を禁じているため、最初の発話が消えて無言でサブエージェント起動に飛ぶ。

**対策（必須）**: 新規のタスク性ユーザー指示への応答では、**最初のツール呼び出しより前に 3〜5 行**
（目的 1 文 + 主要ステップ 2〜4 項目）で「これから何をするか」を報告してから着手する。承認要求では
なく報告なので確認文で終わらせない（CP-6）。同一指示の実行中はフェーズが変わっても再提示しない。
委譲を伴う場合も **到達点（WHAT）は述べ、委譲という手段（HOW）の実況はしない**（§3.1 は維持）。

**判定基準**: 「これは新規タスク性指示への最初の応答か？ まだプラン提示していないか？」→ Yes なら
最初のツール呼び出し前に 1 回出す。境界・発火条件・書式の正本は `output-verbosity-rules.md` §1.1。
**保持理由**: L-111 の過剰適用は圧縮後も全セッションで再発する。Warm 層の SSOT
（`output-verbosity-rules.md`）だけでは圧縮 1 回で新義務が失われるため（Hot 層はディスクから確実に再読込される）。
⚠️ **output style にも同内容があるが、そちらはプロジェクト側で差し替え可能なレイヤ** なので降格しない（#19）。
...
④ **一部だけ観測した値（短縮 SHA 等）を、観測していない部分まで補って完全な値として使う**
（GitHub API へ渡す SHA 系引数〔`expectedHeadSha` / `commitID` 等〕で 3 回連続再発・詳細と
対策は `docs/rules/lessons/pr-review.md` L-160）。
...
再発しうる。`.claude/output-styles/concise-neko.md`（output style）も毎ターン同内容を強制するが、**output style は
プロジェクト側で差し替え可能なレイヤ**（`CLAUDE.md` が口調変更時の書き換えを明記）であり、差し替えで本規律が
失われることを避けるため **Hot 層にも二重で保持する**（#19 で明示決定。3 エントリ中で最も削ってはならない）。
