...
④ **一部だけ観測した値を、観測していない部分まで補って完全な値として使う**（短縮 SHA
`2cea8b4` から 40 桁のフル SHA を creative に組み立てて `expectedHeadSha` / `commitID` 等の API
引数に渡し、実測値と食い違って呼び出しが失敗した・#972 で 3 回連続再発）。

**④ の判定基準**: 完全な値が要るなら、その場で取得し直す（`git rev-parse HEAD` / API の応答
フィールド）。**先頭 N 桁が一致する** ことは残りの桁が一致する根拠にならない。1 回の実測出力を
そのまま使い、他の値と組み合わせて組み立てない。
