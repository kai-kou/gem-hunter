...
#### Layer 1 セルフレビューの修正後に取り直した実測（同日・PR #874）

セルフレビューで **JSON 往復により `Date`（`lastPushedAt`）が文字列に化ける欠陥**（2 段目 HIT でのみ発火し、`Intl.DateTimeFormat` が `RangeError` を投げてページが 500 になる）が見つかったため、封筒に型タグを持たせる修正を入れて再測定した。

| 計測 | 並び | HIT 率 |
|---|---|---|
| 新規キー D を 12 連打 | `MISS HIT HIT MISS MISS HIT MISS MISS MISS HIT MISS HIT` | 5/12（42%） |
| 新規キー E を 12 連打 | `MISS MISS MISS HIT HIT HIT HIT MISS HIT HIT HIT MISS` | 7/12（58%） |

HIT 率は修正前と同水準（＝この修正はキャッシュの効きを変えるものではなく、**HIT したときに壊れた値を返さないための修正**）。あわせて UI ページ（`/ja?q=react`）を 12 回連続で叩き、**全 12 回が 200 で、日付が `2026/09/03` 形式で正しく描画される** ことを確認した（修正前の実装ならこの経路で `RangeError` により 500 になりうる）。

⚠️ **この UI 確認にも §6.1 冒頭と同じ測定の限界がある**: `X-Cache-Status` は UI ページには付与されないため、「2 段目 HIT のレスポンスを描画した」ことの直接確認にはなっていない（`/api/search` で同一 container 経由の HIT が観測できていることからの推論）。層別の可視化は #875 で追う。

🔴 **本節（§6.1）は PR #874 時点（2026-09-03 JST）の記述である。** `infrastructure-design.md` §6.2 の観測条件 2 は、本節時点では依然として充足状態だった: §3.1 の判定式（新規キーへ 12 連打・想定値 75%）に照らすと、上表の 3 本（58% / 58% / 50%）はいずれも 75% を下回っている。本 ADR が条件 2 に対して要求どおり「検討して ADR を起票する」を果たしたことと、条件 2 の **充足状態が解消したこと** は別であり、本節時点ではまだ解消していなかった。**その解消は #875 で追う** → **2026-09-07 JST の §6.2 で解消済み**（本節の記述はその解消前の状態のスナップショットとして残す）。

### 6.2. 層別の可視化と再実測（2026-09-07 JST・PR #1059 のプレビュー・Issue #875）

§6.1 が「層別カウンタを取得して直接証拠にする作業は #875 で追う」としていた残件を実施した。`X-Cache-Status`（`HIT` / `MISS`）は契約を変えずに据え置き、**別ヘッダで段の内訳とコロケーションを併記** する形にした。

| ヘッダ | 値 | 意味 |
|---|---|---|
| `X-Cache-Status` | `HIT` / `MISS` | 既存（変更なし）。§3.1 の判定はこの値で行う |
| `X-Cache-Layer` | `primary` / `secondary` / `miss` | どちらの段で応答したか |
| `X-Cache-Colo` | Cloudflare のコロケーションコード（例 `IAD`） | どのコロケーションで処理されたか |

🔴 **この 3 ヘッダは本番でも出し続ける**（診断が終わったら外す、という扱いにしない）。理由は 2 点:

1. **障害調査の一次手段だから**。キャッシュが効かなくなったときに「どの段まで届いているか」「コロケーションが散っているか」を切り分ける手段が本番に無いと、§6.1 と同じ「推論しかできない」状態へ逆戻りする。環境で出し分けると、本番でだけ観測できない構造ができてしまう
2. **露出する情報が既存の開示範囲を超えないから**。コロケーションコードは Cloudflare 自身が全レスポンスに付ける `cf-ray` ヘッダの末尾セグメント（例 `...-IAD`）として **既に開示されている**。`X-Cache-Colo` はそれを読みやすい形に切り出したもので、新たに秘匿情報を公開するものではない。`X-Cache-Layer` も 2 段構成という設計事実の開示に留まり、キー・値・利用者の情報は一切含まない

⚠️ ただし将来 **キャッシュキーに利用者固有の情報を含める**（ログイン利用者ごとにキーを分ける等）変更を入れるときは、`X-Cache-Layer` の HIT / MISS が「他の誰かが直前に同じ検索をしたか」を示す副チャネルになりうるため、本節の判断を再評価すること。


`https://pr-1059-gem-hunter.kinamocchi-tech.workers.dev/api/search?q=<キー>` へ §3.1 の計測条件（新規キーへ 12 連打）で 3 本測った。

| 計測 | 並び（`X-Cache-Layer`） | HIT 率 | コロケーション |
|---|---|---|---|
| 新規キー A を 12 連打 | `miss, secondary×2, primary×3, secondary×2, primary, secondary, primary, secondary` | 11/12（92%） | 観測前（本節の修正前の版で計測） |
| 新規キー B を 12 連打 | `miss, secondary, primary, secondary×3, primary, secondary×2, primary×3` | 11/12（92%） | 全 12 回 `IAD` |
| 新規キー C を 12 連打 | `miss, secondary×4, primary, secondary, primary, secondary, primary×2, secondary` | 11/12（92%） | 全 12 回 `IAD` |
| 新規キー D を 12 連打（Layer 1 指摘の修正後・下記注記） | `miss, secondary×6, primary, secondary, primary, secondary, primary` | 11/12（92%） | 全 12 回 `IAD` |

🔵 **D 行は Layer 1 セルフレビューの指摘対応後に取り直した値である。** 指摘のうち「`makeObservedCache()` がリクエストごとに `new WorkersCache()` を生成し、`warnedOperations` の isolate 単位 dedup を壊していた」を修正して secondary の実体を共有するようにしたため、キャッシュの実挙動が変わりうる範囲だった。**修正後も 11/12（92%）で同水準** であり、判定は変わらない（secondary HIT の比率は 6 → 8 に増えている）。

🔴 **判定: §6 の 3 分岐のうち「75% 以上」＝ 決着。** 4 本とも 92% で §3.1 の想定値 75% を満たす。したがって **`infrastructure-design.md` §6.2 の観測条件 2 の充足状態は解消した**（§6.1 末尾が「解消は #875 で追う」としていた宿題の回答）。§3 の想定値 75% は **見直さない**（届いたため、Issue #875 の完了条件 2 は発火しない）。

**§6.1 の「測定の限界」も解消した**: 上表のとおり `secondary`（Cloudflare Cache API）での HIT が各本 6〜8 回観測されており、2 段目が働いていることの **直接証拠** になっている（§6.1 の結論は「primary 単独では説明できない改善幅」からの推論に留まっていた）。

**残差（§6.1 の 50〜58%）が消えた理由の考察**: 本計測は（コロケーションを観測できた B・C・D の 3 本とも）全リクエストが単一コロケーション（`IAD`）で処理されており、新規キーの初回に `MISS` が連続する現象は再現しなかった（各本とも `MISS` は 1 回目のみ）。Cache API の共有範囲がコロケーション単位である（§5.2 の最終行）ことと合わせると、§6.1 の残差は **リクエストが複数のコロケーションへ分散していたこと** で説明が付く。実装（2 段構成）を変更した結果ではない（本 PR は観測ヘッダの追加のみで、`LayeredCache` の `get` / `set` の判定順序は変えていない）。したがって **本節の 92% も「コロケーションが分散しない条件での値」であり、利用者が地理的に散れば §6.1 の水準へ戻りうる**（§5.2 の代償として既に受け入れている性質）。

⚠️ **実機知見（実装者向け）**: コロケーションコードは **着信リクエストの `cf-ray` ヘッダからは取れない**。`cf-ray` は Cloudflare が **レスポンスへ** 付けるヘッダで、Worker が受け取る `Request` のヘッダには存在せず、`request.headers.get('cf-ray')` は常に `null` を返す（プレビュー実機で実測）。正しい取得元は `getCloudflareContext()` の `cf.colo` である。単体テストは自分で組み立てた `cf-ray` 値を渡していたため緑のままこの欠陥を通しており、**実機に投げるまで検出できなかった**（`sprint-development-rules.md` `SD-1` の「実機に敵対的入力を投げる」が効いた実例）。
