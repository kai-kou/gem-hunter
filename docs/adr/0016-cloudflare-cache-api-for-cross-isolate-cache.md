...
Issue #121 でプレビュー環境を実測したところ、次の結果になった。

| 計測 | リクエスト | `X-Cache-Status: HIT` | HIT 率 |
|---|---|---|---|
| 同一 URL を 12 回連続 | 12 | 2 | ≒17% |
| 別 URL を 6 回連続 | 6 | 2 | ≒33% |
...
| **想定値** | **定常ヒット率 75%**（12 回中 9 回以上が HIT） |
...
| 同じキー A を続けて 6 連打（定常状態） | `HIT HIT MISS HIT HIT HIT` | 5/6（83%） |
...
**判定: §6 の 3 分岐のうち「17% から改善したが 75% 未満」**（新規キーで 50〜58%・定常状態で 83%）。
