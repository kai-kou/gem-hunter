...
🔴 **判定: §6 の 3 分岐のうち「75% 以上」＝ 決着。** 3 本とも 92% で §3.1 の想定値 75% を満たす。したがって **`infrastructure-design.md` §6.2 の観測条件 2 の充足状態は解消した**（§6.1 末尾が「解消は #875 で追う」としていた宿題の回答）。§3 の想定値 75% は **見直さない**（届いたため、Issue #875 の完了条件 2 は発火しない）。

**§6.1 の「測定の限界」も解消した**: 上表のとおり `secondary`（Cloudflare Cache API）での HIT が各本 6〜7 回観測されており、2 段目が働いていることの **直接証拠** になっている（§6.1 の結論は「primary 単独では説明できない改善幅」からの推論に留まっていた）。

**残差（§6.1 の 50〜58%）が消えた理由の考察**: 本計測は 3 本とも全リクエストが単一コロケーション（`IAD`）で処理されており、新規キーの初回に `MISS` が連続する現象は再現しなかった（各本とも `MISS` は 1 回目のみ）。Cache API の共有範囲がコロケーション単位である（§5.2 の最終行）ことと合わせると、§6.1 の残差は **リクエストが複数のコロケーションへ分散していたこと** で説明が付く。実装（2 段構成）を変更した結果ではない（本 PR は観測ヘッダの追加のみで、`LayeredCache` の `get` / `set` の判定順序は変えていない）。したがって **本節の 92% も「コロケーションが分散しない条件での値」であり、利用者が地理的に散れば §6.1 の水準へ戻りうる**（§5.2 の代償として既に受け入れている性質）。
