...
#### 追補（2026-09-07・Issue #927 実施・実 API 実測）

Issue #170 の完了条件 2（「ETag 経路で 304 を受けたとき、レート枠が減らないことを確認できる」）は PR #926 のマージ時点でユニットテスト（msw モック）のみで、実 API に対する実測が無かった（Layer 1 セルフレビュー Spec 忠実性観点の指摘）。`GET https://api.github.com/repos/kai-kou/gem-hunter` に対し、認証済み（installation token・`Authorization: Bearer $GH_TOKEN`）で 1 回目 GET → 2 回目に `If-None-Match`（1 回目の `ETag` 値）を付けて GET、を `curl -sS -D -` で実行した。
...
|---|---|---|---|---|---|---|
| 連続試行A（間隔ほぼ0秒・認証済み） | 1回目 GET（無条件） | 200 | 8945 | 55 | 1788750576 | `"c7366c4038…"` |
| 連続試行A | 2回目 GET（`If-None-Match` 一致） | **304** | **8945**（不変） | **55**（不変） | **1788750576**（不変） | `"c7366c4038…"`（同一） |
| 連続試行B（間隔ほぼ0秒・`Authorization` ヘッダ未付与） | 1回目 GET（無条件） | 200 | 8944 | 56 | 1788750576 | `"c7366c4038…"` |
| 連続試行B | 2回目 GET（`If-None-Match` 一致） | **304** | **8944**（不変） | **56**（不変） | **1788750576**（不変） | `"c7366c4038…"`（同一） |

**結論**: 認証済み・間隔をほぼ空けない連続試行では、304 応答の前後で `x-ratelimit-remaining` / `x-ratelimit-used` / `x-ratelimit-reset` が **1 単位も変化しなかった**。GitHub 公式記述（「認証済みの条件付きリクエストで 304 が返ると primary rate limit を消費しない」）と実測が一致することを確認した。これにより #170 完了条件 2 は実測ベースで満たされたと判断する。

**観測できなかった事実（正直に記録）**:
- **真の未認証との比較は得られなかった**: 試行 B は `Authorization` ヘッダを付けずに送ったが、応答の `x-ratelimit-limit` が 9000（認証済みティアの値。GitHub の匿名枠は本来 60/時）のままだった。本実行環境のエグレスプロキシ（`$HTTPS_PROXY`・`gitConfigInjection: true` を含む資格情報注入が有効）が `api.github.com` 宛リクエストを透過的に認証している可能性が高く、この環境からは **意図した「未認証」リクエストを再現できなかった**。したがって「未認証時は 304 でも消費される」という GitHub 公式記述そのものの実測検証はできていない（実装判断は公式記述への準拠のまま変更しない）。
- **最初の 1 回だけ行った非連続試行（同一 ETag を約 5 秒後に再送）では `remaining` が 46 減っていた** が、これは 304 という結果に対する消費ではなく、その約 5 秒の間に本セッションの他エージェント（`main` / `ui-tier-decider` 等、並行作業中）が同じ installation token で別の GitHub API 呼び出しを行ったことによる外部要因（ノイズ）だと考えられる。この 1 回だけを見て「304 でも消費される」と結論づけるのは早計であり、上表の間隔ゼロの連続試行（他リクエストが割り込む余地がほぼ無い）を正とする。

**観測スクリプトについて**: `tools/github_rest.py` を着手前に確認したが、`http_get()` は追加ヘッダ（`If-None-Match`）の付与にも応答ヘッダ（`x-ratelimit-*` / `etag`）の取得にも対応しておらず、本タスクのような「レスポンスヘッダそのものを比較する 1 回限りの観測」という同型の役割を持つ関数は存在しない（該当なしを確認した）。今回の観測は 1 回限りの検証作業であり、恒久的に再実行する仕組みは要求されていない（`docs/rules/sprint-development-rules.md` の TDD 対象は「振る舞い」であって使い捨ての手動観測ではない）ため、`tools/` への恒久スクリプト追加は見送り、上記の curl コマンドと実測結果をこの追補に記録することで再現手順を残す。
