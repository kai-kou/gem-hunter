...
installation token が広げるのは **レート枠だけではない**。App のインストール先に含まれるプライベートリポジトリは、`GET /search/repositories` や `GET /repos/{owner}/{repo}` の応答に **正当な結果として現れうる**（未認証では存在しないのと同じだった）。§4 で実測したのは「他人の公開リポジトリを読めること」であり、**「公開リポジトリしか読めないこと」は検証していない**。

本プロダクトの対象は公開リポジトリのみ（[PRD](../02_requirements/prd.md) §3.1）であるため、**可視範囲の限定を App の権限設定・インストール先の運用に委ねず、アプリ側で明示的に強制する**（`NFR-33` / `AC-12`）。

1. 検索クエリに公開限定条件（`is:public`）を必ず AND 付与する
2. 検索結果のマッピングで `private: true` の項目を除外する
3. `GET /repos/{owner}/{repo}` の応答が `private: true` なら「見つからない」として扱う

🔵 **本 ADR の決定（§2）は変更しない。** 変わるのは「App を採ったことで生じた責務」の明示であり、この 3 層の強制はデータアクセス層（`src/infrastructure/github/`）の実装要件として扱う。
