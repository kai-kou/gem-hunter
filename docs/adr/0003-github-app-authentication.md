...
> 🔵 **本プロジェクトでの決め手は §3.2（運用の手離れ）であり、枠の拡大は付随して得られた利点**（MVP のトラフィックでは PAT の 5,000 req/h でも足りる）。並び順は影響の大きさ順ではない（§6.1 軸 1）。
...
### 5.4. 🔴 新たに負う責務: 認証はデータの可視範囲も広げる（2026-08-19 追記）

installation token が広げるのは **レート枠だけではない**。App のインストール先に含まれるプライベートリポジトリは、`GET /search/repositories` や `GET /repos/{owner}/{repo}` の応答に **正当な結果として現れうる**（未認証では存在しないのと同じだった）。§4 で実測したのは「他人の公開リポジトリを読めること」であり、**「公開リポジトリしか読めないこと」は検証していなかった**。

🔴 **本節を追記した契機**: リポジトリオーナー（ユーザー）から **「自分のプライベートリポジトリが検索対象になっている」という報告** があり、それを受けて調査・修正した。⚠️ **こちら側で API 応答を採取して混入を実測した記録は残していない** ため、本 ADR も教訓（`L-128`）も「実測で混入を確認した」とは書かない（発覚経路はユーザー報告）。

本プロダクトの対象は公開リポジトリのみ（[PRD](../02_requirements/prd.md) §3.1）であるため、**可視範囲の限定を App の権限設定・インストール先の運用に委ねず、アプリ側で明示的に強制する**（`NFR-33` / `AC-12`）。

1. **利用者キーワードに含まれる修飾子構文（`名前:値` / `-名前:値`）を拒否する**（検索クエリへのインジェクション遮断）
2. 検索クエリに公開限定条件（`is:public`）を付与する
3. 検索結果のマッピングで `private: true` の項目を除外する
4. `GET /repos/{owner}/{repo}` の応答が `private: true` なら「見つからない」として扱う

🔴 **`is:public` の付与だけでは不十分である。** GitHub の検索構文は同一修飾子の重複指定（`is:private is:public`）を AND として解釈しないため、利用者がキーワードに `is:private` や `user:誰か` と書けるままだと第 2 層は破れる。**キーワード側の修飾子構文を遮断する層（1）が前提** であり、「`is:public` を AND 付与すれば公開に閉じる」という理解を残さない。

🔵 **検索での認証は維持する**（未認証検索へ切り替えない）。切り替えるとレート枠が 30 req/分 → 10 req/分に下がり、アプリの律速（§3.1）が悪化するため、可視範囲の限定は上記のアプリ側 4 層で行う。

🔵 **本 ADR の決定（§2）は変更しない。** 変わるのは「App を採ったことで生じた責務」の明示であり、この 4 層の強制はデータアクセス層（`src/infrastructure/github/`）の実装要件として扱う。

🔵 **一般化**: 外部 API の認証を追加・強化する決定は、レート枠・信頼性・秘密の取り回しだけで評価しない。**取得しうるデータの可視範囲がどう変わるか** を同じ表で必ず評価する（`L-128`）。
...
|---|---|---|---|---|
| **1** | **Core 枠 5,000 req/h で足りるか** | 足りる | 足りない・余裕が欲しい（`5,000 + 20 を超えたリポジトリ 1 件につき 50` / 上限 12,500） | **MVP のトラフィックでは足りる**（決め手ではない。実際の決め手は軸 3）→ 余裕として **App** |
| **2** | 🔴 **トークン取得の往復コストを払えるか** | 払えない（呼び出しが低頻度・少数で、1 時間ごとの取得往復が相対的に重い） | 払える（呼び出しが多い / トークンを失効前まで再利用できる） | 継続的にリクエストが出るサーバー → **App** |
...
🔴 **軸 2 と軸 4 は拒否権を持つ**。どちらかが「No」なら、枠（軸 1）や運用（軸 3）の優位は意味を持たない — 取得の往復が相対的に重い用途では installation token の 1 時間 TTL は **利点ではなく障害** になり、実装量を増やせない状況では JWT 署名の手間が丸ごとコストとして残る。本プロジェクトは両方が「Yes」だったため App を採れた。

⚠️ **軸 2 を「実行形態」で判定しない**（単発ジョブだから App は無理、という切り方をしない）。署名鍵さえ渡せれば単発の CI ジョブや cron でも App 認証は成立する（GitHub 公式の `actions/create-github-app-token` がその形）。効いてくるのは **1 リクエストあたりに取得往復が何回乗るか** という比であって、常駐しているかどうかではない。

### 6.2. 決め手にならなかった軸（誤解しやすいので明記する）

> 🔵 §6.1 は **条件次第で差がつく軸**（本プロジェクトではたまたま差がつかなかったものを含む）。本節は **どんな条件でも方式の差にならない軸**。

| 軸 | 実態 |
|---|---|
| **検索 API の枠** | 🔴 **両者とも 30 req/分で同じ**。アプリの律速はここなので、**App にしても検索は速くならない**（§3.1 の注記と同じ） |
| 他人の公開リポジトリを読めるか | 両者とも読める（App 側は 2026-08-18 に実測確認済み・§4） |
| ETag の効き | 公式は認証方式を区別せず「正しく認証された条件付きリクエストの 304 は消費しない」としている。**App 側は実測済み（§3.4）・PAT 側は未実測** |
| 金銭コスト | どちらも無料 |
...
2. GitHub App の Core 枠が 5,000 req/h まで縮小した場合（インストール先のリポジトリ数の変動で起こりうる）→ 軸 1（枠）の優位が消える。⚠️ ただし **本決定の決め手は軸 3（運用の手離れ・§3.2）なので、枠の縮小だけでは撤回に至らない**。軸 4（実装コスト）が同時に重くなったときに PAT を再評価する（§6.1）
