...
| 未確定パラメータ | 決定に必要なもの |
|---|---|
| ~~パーセンタイルの母集団定義（`Q-3` 従属事項）~~ 🔴 **`D-37`（2026-08-22）で確定**（レジストリ別成層化 + 汚染フィルタ + flagship dedupe） | ~~対象エコシステム（`D-10`）ごとの実データ分布~~ 実データ 18 万パッケージで確定済み |
| `Gem Index` のしきい値・足切り基準の具体値 | 実データのサイズと分布（[PRD](../02_requirements/prd.md) §13） |
| 配信件数（Gem 一覧の表示数） | 同上 |
| ~~対象エコシステムの最終確定（`D-10` は npm + PyPI を推奨に留める）~~ 🔴 **`D-37`（2026-08-22）で確定**（12 レジストリ） | ~~Ecosyste.ms のカバレッジ実測~~ 実測済み（[リサーチ記録](../01_research/data/20260822-dependency-data-sources.md)） |
...
>
> 🔴 **`D-36`〜`D-38`（2026-08-22）追記**: 母集団を Ecosyste.ms の **12 レジストリ・109,469 リポジトリ** へ拡大した実測により、`D-33` の 2 つの撤去根拠（npm 限定という構造的限界 / 10 万件がバンドル 3MB を圧迫する）は **いずれも解消した**（検索上位 100 件の被覆は react 32% / test framework 36% / image processing 19%・検索ヒットの 52% が非 npm）。ただし **`sort=gem-index`（検索結果の主ソート軸）は復活させない**（`D-33` のこの部分は維持）— 100 件中 68 件に値が無くソート順を与えられず、かつ Gem Index は定義上「有名なものを下げる」ため検索の関連度を壊すため。**採用する形はバッジ + 検索語連動の Gem 一覧**（一覧内では全件が Gem なので順序が成立する）。あわせて §2.3 の未確定パラメータのうち **パーセンタイルの母集団定義** は `D-37`（レジストリ別成層化 + 汚染フィルタ + flagship dedupe）で確定した。詳細は [`open-questions.md`](../02_requirements/open-questions.md) `D-36`〜`D-38` と[リサーチ記録](../01_research/data/20260822-dependency-data-sources.md)を参照
