...
- [ ] `M-5'`: マージ済み `claude/*` ブランチを削除
- [ ] Wiki / Projects の中身を確認（空でなければ内容を確認）

### Phase 1.5: `M-4` の通過判定（プレビュー URL ごと公開する選択の前提）

- [ ] `R-8`: GitHub 利用規約 / AUP / API Terms の該当節を一次確認（アバター画像の再配信可否を含む）
- [ ] `R-5`: 必要レート枠の逆算と、それに基づくキャッシュ TTL の確定
- [ ] `R-6`: 運用コストが無料枠に収まることの試算
- [ ] Issue #187: プレビュー version に secret が渡らず、認証とレート制限が効いていない状態を解消する
...
### 7.3. `R-6`: 運用コストの試算 — ⚠️ 条件付き完了（プランの実機確認が残る）
...
**🔴 残る確認事項（ユーザーのみ実施可能）**: `wrangler.jsonc` に `limits.cpu_ms: 50` が設定されているが、**この設定は Workers Paid 向け** である。実アカウントが Free と Paid のどちらかによって公開のリスクが変わる。

- **Free の場合**: 上記のとおりコスト上限は構造的に 0 円。公開して問題ない
- **Paid の場合**: 🔴 **リクエスト数課金を止める native な手段が存在しない**（§5.4）。公開でトラフィックが増えると課金が伸びうるため、Billable Usage API の日次ポーリングによる監視を先に入れる

セッションの API トークンには課金情報を読む権限が無く（`subscriptions` エンドポイントが Authentication error）、**プランを機械的に判定できない**。ダッシュボードでの確認をユーザー作業として切り出す。

### 7.4. Issue #187: プレビューに secret が渡っていない — ⚠️ 未解消

プレビュー version に secret が投入されておらず、**認証（GitHub App installation token）とレート制限がプレビューで動作しない**。この状態のプレビュー URL を公開すると、

- 未認証で GitHub API を叩くため **レート枠が 10 req/分** まで落ちる（認証済みの 1/3）
- クライアント単位のレート制限（`NFR-7`）も効かない

`M-4` の通過判定そのものには含まれないが、**「URL ごと公開する」判断と組み合わさると実害が出る** ため、公開前に解消するか、露出しているプレビュー version を破棄する。
