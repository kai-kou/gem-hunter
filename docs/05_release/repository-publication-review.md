...
⚠️ **`SP-16`（`sort=gem-index`）は本逆算の前提が成り立たない（PR #293 セルフレビュー指摘）**: 上表「1 検索あたりの上流呼び出し」は **1 回** を前提にしているが、`sort=gem-index` は内部で最大 10 ページを逐次取得するため **1 検索あたり最大 10 リクエスト** になりうる（`src/usecases/search-repositories.ts` の `GEM_INDEX_FETCH_MAX_PAGES`）。**本節では再逆算しない**（別途行う）。
