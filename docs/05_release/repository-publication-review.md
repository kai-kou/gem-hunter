...
- [x] `R-6`: 運用コストの試算 → Free である限り構造的に 0 円。⚠️ **プラン（Free / Paid）の実機確認だけ残る**（§7.3・ユーザー作業）
...
### 7.6. ブランチ削除がセッションから実行できない（実測 2026-08-20）

`M-5'`（マージ済み `claude/*` ブランチ 40 本の削除）は、**本セッションからは実行できない** ことが実測で分かった。

| 経路 | 結果 |
|---|---|
| `git push origin --delete <branch>` | 🔴 `error: RPC failed; HTTP 403`（git プロキシが削除 refspec を拒否） |
| `DELETE /repos/{owner}/{repo}/git/refs/heads/<branch>`（REST 直叩き） | 🔴 `403` |
| GitHub MCP | 🔴 **ブランチ削除に相当するツールが存在しない**（`create_branch` はある） |

> 🔵 参考: 同じセッションから `GET /repos/.../issues` 等の **読み取り REST は 200 で通り**、MCP 経由の PR 作成・マージ・レビュー投稿も成功する。つまり「REST 全般が 403」ではなく、**書き込み系の直叩きが遮断され、MCP が提供する操作だけが通る** 構造である（`lessons-core.md` L-114 の実測補強）。

→ ユーザー作業として切り出す。ローカルの `gh` / `git` からは通常どおり削除できる。
