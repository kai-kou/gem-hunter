...
**🔴 プランの実機確認（2026-08-20 実施・飼い主がダッシュボードで確認）**: 本アカウントは **Workers Paid（$5/月 + 従量）** である。`wrangler.jsonc` の `limits.cpu_ms`（🔵 `SP-18`（#388）の cold start 実測により **50 → 200** へ引き上げ済み・根拠は `D-38` の `SP-18` 実測追記）が有効に働く一方、`INF-2` が想定した「超過したら課金されず停止する」構造は **効いていない**。
...
⚠️ **残余リスク（意図的な濫用）**: 日 89 万リクエストを継続的に浴びせられれば撤退ラインに触れる。`*.workers.dev` は自分のゾーンに属さないため **Cloudflare の WAF レート制限ルールを適用できず**、アプリ内の `RATE_LIMITER` は Worker が起動した後に効くので **リクエスト数課金そのものは止められない**。`D-19` が約束した Billable Usage API の日次ポーリングは **実装・実データでの疎通確認とも完了**（`tools/check_cloudflare_cost.py`・Issue #247 で実装、Issue #1016 で Billing Read 権限を付与、Issue #939 / PR #1101 で応答形式を実測して実装を追随）。したがって本残余リスクは **低減されている** が、検知は日単位のため **同一日内の一部内訳行の欠落は検出できない**（既知の限界）。仕様と制約の正本は [`cloudflare-infrastructure.md`](../03_design/infrastructure/cloudflare-infrastructure.md) §5.5。

→ **公開の阻害要因ではない**。`D-19` の約束はツール実装（Issue #247）で判定ロジックを満たし、🟢 **実データでの監視も稼働している**（Billing Read 権限の付与は Issue #1016 で完了・2026-09-08 JST）。
