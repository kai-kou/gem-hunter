...
⚠️ **残余リスク（意図的な濫用）**: 日 89 万リクエストを継続的に浴びせられれば撤退ラインに触れる。`*.workers.dev` は自分のゾーンに属さないため **Cloudflare の WAF レート制限ルールを適用できず**、アプリ内の `RATE_LIMITER` は Worker が起動した後に効くので **リクエスト数課金そのものは止められない**。`D-19` が約束した Billable Usage API の日次ポーリングは **判定ロジックのみ実装済み**（`tools/check_cloudflare_cost.py`・Issue #247）で、**実データでの疎通は未確認**（現行トークンに Billing Read 権限が無く「判定不能」へ倒れる）。したがって本残余リスクは **まだ低減していない**。仕様と制約の正本は [`cloudflare-infrastructure.md`](../03_design/infrastructure/cloudflare-infrastructure.md) §5.5。

→ **公開の阻害要因ではない**。`D-19` の約束はツール実装（Issue #247）で判定ロジックまで満たしたが、**実データでの監視が始まるのは Billing Read 権限の付与後** である。⚠️ 権限付与は `A-6`（現行トークンは `subscriptions` エンドポイントで Authentication error）。
