...
>
> ルーティン（`sprint-cycle-router` の 2 時間ごとの firing）が **Step 5（バックログ消化）で本ファイルを読み、§2 の表の順に 1 件ずつ着手する**。順序を変えたいときは本ファイルの表の行を入れ替える（Issue のラベルや SKILL.md をその都度いじらない）。
...
| **5.5** | **#543** | 🔴 **`status:waiting-user`**（実装保留・[Issue コメント](https://github.com/kai-kou/gem-hunter/issues/543#issuecomment-5388749590)参照）。`D-23`/`D-40`（品質チェックには Actions を使わない・飼い主決定 #298）と直接衝突する恒久方針変更のため、無人 firing では続行せず飼い主確認を待つ | push/PR 契機の CI 追加。ただし D-23/D-40 との整合が取れるまで着手しない |
| **5.8** | **#544** | README 冒頭から与件充足チェックリストへの導線が無い（[`minimum-requirements-checklist.md`](../02_requirements/minimum-requirements-checklist.md) は存在するが README から 1 クリックで到達できない） | 評価者が与件充足を確認するまでの導線が長い。コストが小さく効果が大きい |
...
**解除作業の実体は #466**（§2 の表の最終行）。以下をすべて満たしたら、本ゲートの運用を終了する。

- [ ] §2 の表の全行（#466 を除く）が closed で、`npm run check` の E2E が緑
- [ ] `release:required` / `release:deferred` の 2 ラベルを全 Issue から剥がし、ラベル自体を削除する
- [ ] ゲート由来の追記を撤去する: `sprint-cycle-router` SKILL.md（Step 5 の 2 箇所・§2 早期リターン c）/ `self-improvement-loop` SKILL.md（上書き契約の 1 行・Step G-1.5 の除外 1 行）/ `CLAUDE.md`（ラベル 2 行）/ `.claude/commands/next.md`（除外 1 行）/ `docs/rules/improvement-lane-map.md`（参照 1 行）
