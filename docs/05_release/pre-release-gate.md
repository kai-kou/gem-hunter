...
> **このファイルは「提出（リリース）前に片付ける Issue と、その着手順」の正本 **だった**（Issue #452・2026-08-23〜08-24）。**
>
...
| **8** | **#466** | 対応済み（PR #585）。`release:required` / `release:deferred` を全 Issue（31 件）から剥がし、ラベル定義も削除したうえで、`.claude/` / `CLAUDE.md` / `docs/rules/` からゲート参照を撤去した | 🔴 **解除の発火主体**。これを表の最終行に置くことで、`release:required` が空になる直前に必ず 1 件残り、ルーティンが自力で解除に到達した（置かないと終了済みゲートを毎 firing 読み続けることになっていた） |
...
- [x] §2 の表の全行（#466 を除く）が closed で、`npm run check` の E2E が緑（2026-08-24 JST・最後の 1 件 #543 完了）
