...
初回分析（2026-06・Issue base#2860）の再現可能版。カテゴリルールは本ツールが正本であり、
...
追加したい場合はコード変更不要で同ファイルに追記できる（base#420）。
...
実行タイミング: 毎週月曜の 07:00 スロット ⑤.7（{プロジェクト定義: hourly-routing 相当}・週次化 base#2900）
...
REPO = resolve_repo_slug("kai-kou/gem-hunter")
...
# で行う（Issue base#627 対策 E）。PLAUSIBLE のインライン化は base#627 で廃止したが、それ以前の投稿を検出できるよう
# 確度の選択肢に残す。
...
# ときのフォールバック。内容は同ファイルの既定値と同一に保つ・base#420）。
...
def load_category_rules(path: Path = CATEGORY_RULES_CONFIG) -> "list[tuple[str, str, str]]":
    ...
    このファイルに `categories` エントリを追記するだけでよい（コード変更不要・base#420）。
    """
    try:
...
def is_layer1_finding(body: str) -> bool:
    """本文が Layer 1 セルフレビューの指摘テンプレートか（既定の AI 指摘判定・base#627 対策 E）。"""
...
def run_self_test() -> int:
    """AI 指摘の判定（Layer 1 テンプレート / 旧ログイン名）と severity 抽出の内蔵テスト（base#627 対策 E）。"""
    ok = True
