...
## #476（Cloudflare 側の同型問題）との関係（Issue #602 対応方針5・決定の記録）

**意図的に分ける**（同じ設計へ揃えない）。GitHub REST は「返ってきた件数が `per_page` 未満なら
最終ページ」という **結果から推定する** 継続判定で、総件数を事前に知る手段が無い。Cloudflare API は
応答直下の `result_info`（`page`/`per_page`/`count`/`total_count`）で **総ページ数が明示される**。
この差はレスポンス仕様そのものに由来し、共通の抽象へ寄せると「使われない方の分岐」を常に抱える
（YAGNI 違反）。したがって Cloudflare 側は #476 の中で独立に集約し、本モジュールへは寄せない。
将来 Cloudflare 側を集約するときは、本モジュールの `fetch_page` コールバック方式（HTTP 層を
呼び出し元に残し、継続判定だけを共通化する）という **形だけを踏襲してよい**（実体は共有しない）。
