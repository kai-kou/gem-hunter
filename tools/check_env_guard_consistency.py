...
終了コード:
  0 = 合格（矛盾なし）
  1 = 矛盾あり（deny の名前が第2層でブロックされない／ひな形が deny にも列挙されている 等）
...
# lib/env_allowlist.sh の hook_env_guard_template_names() が返す固定名と一致させる
# （二重管理を避けたいところだが、Python から bash 関数の中身を実行時に取得して突き合わせる
#  ことで drift を機械検出する。ここではフォールバック値として保持する）
FALLBACK_TEMPLATE_NAMES = [".env.example", ".env.sample", ".env.template", ".env.dist"]

DENY_ENV_PATTERN = re.compile(r"^Read\((\.env[^)]*)\)$")
...
def load_deny_names(settings_path: Path) -> list[str] | None:
    """settings.json の permissions.deny から `Read(.env...)` 系パターンを抽出する。
    戻り値: パターン文字列のリスト（0件含む）。読み込み・パース自体に失敗したら None。
    """
...
def check_consistency(
    ...
    if tmpl_names is None:
        tmpl_names = FALLBACK_TEMPLATE_NAMES
...
def run_checks(
    ...
    deny_names = load_deny_names(settings_path)
    if deny_names is None:
        return 2, [f"settings.json を読み取れない・パースできない（判定不能）: {settings_path}"]
    return check_consistency(deny_names, lib_path, runner=runner)
...
def self_test() -> int:
    ...
    # --- 0) 前提: 実データを読み込めること ---
    real_deny = load_deny_names(SETTINGS_PATH)
    if real_deny is None:
        failures.append("実際の settings.json から deny を読み込めなかった（自己診断不能）")
        real_deny = []
    real_tmpl = template_names(LIB_PATH, runner=recording_runner) or FALLBACK_TEMPLATE_NAMES
    ...
    if LIB_PATH.is_file():
        ...
        backup_path = Path("/tmp") / "env_allowlist.sh.pre-mutation.bak"
        backup_path.write_text(original, encoding="utf-8")
        try:
            # 事前条件: 置換対象の文字列が源文に存在するか
            target = ".env.example|.env.sample|.env.template|.env.dist|.env.example.*) return 1 ;;"
            if target not in original:
                failures.append("変異テストの事前条件不成立: 置換対象の文字列が源文に見つからない")
            else:
                mutated = original.replace(target, ".env.example|.env.sample|.env.template|.env.dist|.env.example.*) return 0 ;;")
                # 事後条件: 置換で内容が変わったか
                if mutated == original:
                    failures.append("変異テストの事後条件不成立: 置換しても内容が変わらなかった")
                else:
                    LIB_PATH.write_text(mutated, encoding="utf-8")
                    v = hook_verdict(".env.example", LIB_PATH, runner=recording_runner)
                    # 変異後は「ひな形なのにブロックされる」ため、check_consistency が違反検出するはず
                    mcode, mviol = check_consistency(real_deny or [".env"], LIB_PATH, runner=recording_runner)
                    if v is not True or mcode != 1:
                        failures.append(
                            f"変異テスト（ひな形の分岐を反転）が self-test を FAIL させなかった: "
                            f"verdict={v} check_consistency_code={mcode} violations={mviol}"
                        )
        finally:
            LIB_PATH.write_text(original, encoding="utf-8")
            diff = subprocess.run(
                ["git", "diff", "--exit-code", str(LIB_PATH)],
                cwd=REPO_ROOT, capture_output=True, text=True,
            )
            if diff.returncode != 0:
                failures.append(f"変異を戻したのに残骸が残っている: {diff.stdout}")
    else:
        failures.append("変異テスト対象の env_allowlist.sh が見つからない")

    # --- 8) fake runner の argv 検証（#710） ---
    if not calls:
    ...
    # --- 9) main() からの実到達（エントリポイント経由の exit code） ---
    proc = subprocess.run(
