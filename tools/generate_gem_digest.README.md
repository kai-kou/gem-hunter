...
| 項目 | 実測値 |
| --- | --- |
| 実行時間（12 レジストリ × 15,000 件） | **2m 37s**（別実行では 8m 07s。Ecosyste.ms 側の応答時間に左右される） |
| 総リクエスト数 | **180**（12 レジストリ × 15 ページ・`per_page=1000`。匿名 5,000 req/時の枠内） |
| 総取得パッケージ数 | **180,000** |
| プール件数（ユニーク repo 数） | **62,565** |
| シャード合計サイズ（raw） | **3.48 MiB**（12 ファイル） |
| 最大シャードのサイズ（raw） | **564.7 KiB**（`proxy-golang-org.json`） |
| `daily-digest.json` のサイズ | **66.5 KiB**（上位 300 件） |
| レジストリ別構成比（上位 3 件） | npm 15.48% / PyPI 14.92% / Go 13.50%（最小は CPAN 1.55%） |
| 除外理由別件数 | star 欠損 13,750 / 被依存 0 件 14,344 / 低 star の汚染疑い 29,557 / repo 重複 27,311 |
...
- 🔵 **Gem Index 上位 20 件に `stars=0` の自動生成ミラー・repo 誤紐付けは 0 件**（`D-36` の失敗判定条件は「3 件以上混ざっていない」）
- 被覆率の再測定は `python3 tools/measure_gem_coverage.py` で行う（結果は `open-questions.md` `D-36` の追記に記録する）
