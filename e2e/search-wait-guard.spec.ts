 ...
 *
 * 🔵 **この予算は flakiness 対策であって、欠陥検知の主軸ではない**（PR #1064 の Layer 1
 * セルフレビュー指摘）。`searchFor()` がナビゲーション完了を待たずに戻る退行（Issue #624 の
 * 欠陥そのもの）の検知は、下の「応答が 1.5 秒遅い」テストが持つ `MIN_ELAPSED_MS`（1,200ms・
 * 本値の変更に影響されない）が担う。「高速応答」テストが担保するのは「戻った時点で結果が
 * 描画済みである」ことであり、その判定に許す猶予をここで決めている。予算を絞りすぎると
 * React のバッチ化で偽陽性になり、緩めすぎると描画完了の滑り込みを見逃す。
 */
