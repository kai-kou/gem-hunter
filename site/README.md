...
- 🔴 **footer から README 本体（`blob/main/README.md`）へ 1 クリックで到達できる状態を保つ**（README → LP のリンクとの非対称を止めるため・Issue #403）。`tools/check_site.py` が機械検査するので、footer を整理するときにこのリンクを消さない
