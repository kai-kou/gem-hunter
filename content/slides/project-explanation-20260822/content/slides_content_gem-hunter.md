...
- 本文:
  - 検索結果のカードには、Gem Index（被依存数と star の順位差。定義は後半で扱う）が高い候補に「Gem」の印が付く
...
- 本文:
  ...
  - 検査は 38 項目。lint・型・テスト・E2E・Lighthouse に、依存規則などの静的検査と運用ツール自身の self-test を重ねてある
...
- 本文:
  ...
  - それでも sort=gem-index は戻さない。100 件中 68 件に値が無く、主ソートには使えない
...
- 本文:
  ...
  - 採用: Gem のデータも静的 JSON で、サーバー側に状態を持たない点は他と同じ
