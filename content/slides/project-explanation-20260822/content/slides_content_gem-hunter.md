...
対象読者: 開発者・エンジニア
想定尺: 21〜24 分（21 枚 / 1 枚あたり 60〜70 秒。スライド 17 のみ 90〜100 秒）
作成日: 2026-08-22（2026-08-23 改訂）
...
- 本文:
  - 検索結果のカードには、star のわりに多くのパッケージから使われている候補に「Gem」の印が付く
  - 印が付いた候補だけを見たくなったら、検索結果の上の導線から Gem 一覧へ移れる
  - 一覧は検索語をそのまま引き継ぎ、過小評価の度合いが高い順に 20 件ずつ並ぶ
  - 絞り込みはリポジトリ名とパッケージ名の単語境界一致で、0 件のときだけ 1 語に緩める
  - 印が付いた候補は URL に同伴させて渡すので、印と一覧で判定基準がずれない
  - 印が付かないことは評価が低いことを意味しない、と画面にも明記している
  - この一覧は GitHub API を叩かないので、レート制限の枠を検索とは別に用意した
...
- 本文:
  ...
  - Gem Index は検索結果の並び順を変えない。順位づけに使うのはこの並びと Gem 一覧だけ
...
- 本文:
  ...
  - いまの規模は単体・結合が 779 ケース、E2E が 106 ケース（2026-08-23 時点）
...
- 出典: docs/04_development/testing-strategy.md / sprint-development-rules.md SD-2 / テストファイルの実測（2026-08-23）
...
- 本文:
  ...
  - 検査は 38 項目。lint・型・テスト・E2E・Lighthouse に静的検査 14 点を重ねる
...
- 本文:
  ...
  - 当時の候補プールは npm だけを被依存数の降順に取った上位 294 件だった
...
- 本文:
  - 撤去の理由は候補が 227 件しかないことで、指標そのものの否定ではなかった
  - 集める先を npm 1 つから 12 レジストリへ広げ、各 15,000 件の固定枠で集め直した
  - star 欠損・被依存 0 件・低 star の汚染・重複を落として、ユニーク 62,483 リポジトリが残る
  - 1 レジストリ支配は起きず、最大の npm でも構成比は 15.5% に収まる
  - 一般語 24 語で測り直すと、検索上位 100 件との被覆率は平均 34.5%・0 件の語はなし
  - 3 MB のバンドル上限も、静的アセットとして配る方式に変えたことで当たらなくなった
  - それでも sort=gem-index は戻さず、印と一覧という別の形で検索経路へ入れ直した
...
- 本文:
  ...
  - 採用: Gem のデータも日次バッチが作った静的 JSON をリポジトリに置いて配る
...
- 本文:
  ...
  - 判断の根拠と却下案を残しておくと、前提が変わったときに測り直して戻せる
